Building the kpl installer

Note: all pathes are relative to the git repository folder (KPL-private)
...
Test X11 installer: java -jar KPL-client-<version>.jar
Test console installer: java -jar KPL-client-<version>.jar -console
...
Copy the content of Java JRE folder (C:\Program Files\Java\jre<version>) into the jre folder under your KPL-private git repository preserving the folder structure
...
Test the installer using Java: java -jar KPL-client-<version>.jar
Test the windows executable: KPL-client-<version>.exe
...
kpl.jar is now self executing, the full classpath is embedded into the jar file manifest automatically by jar.sh
