...
### Database ###
  ...
  - main database: `kpl_db/`
  - test database: `kpl_test_db/`
  ...
  - use the kpl.db framework when accessing the database from your code
...
### Where to Look ###
  ...
  - js/krs.*.js: corresponding js files for each task the file name displays, one roughly for each page
  - js/krs.modals.*.js: The modal js (popups) for each set of popups, one for each set of modals
...
- https://crowdin.com/project/KPL-ui-translation
...
4. New languages can be added to ``krs.languages`` in ``ui/js/krs.settings.js`` file. Review the status of the languages (40-50%+ Experimental, 70-80%+ Beta, 90-95%+ Stable), eventually add new languages
