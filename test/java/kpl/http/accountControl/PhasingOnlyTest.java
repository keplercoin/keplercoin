...
import kpl.Kpl;
import kpl.PhasingParams;
import kpl.VoteWeighting.MinBalanceModel;
import kpl.VoteWeighting.VotingModel;
import kpl.http.APICall;
import kpl.http.APICall.Builder;
...
public class PhasingOnlyTest extends BlockchainTest {
    ...
    public void testAccountVoting() throws Exception {
        ...
        Builder builder = new ACTestUtils.Builder("sendMoney", ALICE.getSecretPhrase())
            .recipient(BOB.getId())
            .param("amountNQT", 1 * Constants.ONE_kpl);
    ...
    public void testExtraRestrictions() throws Exception {
        //all transactions must be approved either by BOB or CHUCK, total fees 5 kpl, min duration 4, max duration 100
        setPhasingOnlyControl(VotingModel.ACCOUNT, null, 1L, null, null, new long[] {BOB.getId(), CHUCK.getId()},
                5 * Constants.ONE_kpl, 4, 100);

        Builder builder = new ACTestUtils.Builder("sendMoney", ALICE.getSecretPhrase())
                .recipient(BOB.getId())
                .param("amountNQT", 1 * Constants.ONE_kpl)
                .feeNQT(7 * Constants.ONE_kpl);
        // fee too high
        setTransactionPhasingParams(builder, 20, VotingModel.ACCOUNT, null, 1L, null, null, new long[] {BOB.getId(), CHUCK.getId()});
        ACTestUtils.assertTransactionBlocked(builder);

        // fee at the limit
        builder.feeNQT(5 * Constants.ONE_kpl);
        ...
        builder.param("phasingFinishHeight", Kpl.getBlockchain().getHeight() + 200);
        ACTestUtils.assertTransactionBlocked(builder);
        builder.param("phasingFinishHeight", Kpl.getBlockchain().getHeight() + 3);
        ACTestUtils.assertTransactionBlocked(builder);
        builder.param("phasingFinishHeight", Kpl.getBlockchain().getHeight() + 4);
        ACTestUtils.assertTransactionSuccess(builder);
    ...
    public void testRejectingPendingTransaction() throws Exception {
        ...
        Assert.assertEquals(balanceBeforeTransactionRejection + 1 * Constants.ONE_kpl,
                ACTestUtils.getAccountBalance(ALICE.getId(), "unconfirmedBalanceNQT"));
    }
    
    @Test
    public void testBalanceVoting() {
        setPhasingOnlyControl(VotingModel.NQT, null, 100 * Constants.ONE_kpl, null, null, null, 0, 0, 0);
        ...
        setTransactionPhasingParams(builder, 20, VotingModel.NQT, null, 100 * Constants.ONE_kpl, null, null, new long[] {DAVE.getId()});
        ACTestUtils.assertTransactionBlocked(builder);
        
        setTransactionPhasingParams(builder, 20, VotingModel.ACCOUNT, null, 1L, null, null, new long[] {BOB.getId(), CHUCK.getId()});
        ACTestUtils.assertTransactionBlocked(builder);
        
        setTransactionPhasingParams(builder, 20, VotingModel.NQT, null, 100 * Constants.ONE_kpl + 1, null, null, null);
        ACTestUtils.assertTransactionBlocked(builder);
        ...
        setTransactionPhasingParams(builder, 20, VotingModel.NQT, null, 100 * Constants.ONE_kpl, null, null, null);
        ACTestUtils.assertTransactionSuccess(builder);
    }
    ...
    private void setTransactionPhasingParams(Builder builder, int finishAfter, VotingModel votingModel, String holdingId, Long quorum,
        ...
        builder.param("phasingFinishHeight", Kpl.getBlockchain().getHeight() + finishAfter);
