...
import kpl.util.CountingInputReader;
import kpl.util.CountingInputStream;
import kpl.util.CountingOutputWriter;
...
final class PeerImpl implements Peer {
    ...
    void setVersion(String version) {
        ...
        isOldVersion = false;
        if (Kpl.APPLICATION.equals(application)) {
            String[] versions;
    ...
    public int getWeight() {
        ...
        if (hallmarkBalance == -1 || hallmarkBalanceHeight < Kpl.getBlockchain().getHeight() - 60) {
            long accountId = hallmark.getAccountId();
            Account account = Account.getAccount(accountId);
            hallmarkBalance = account == null ? 0 : account.getBalanceNQT();
            hallmarkBalanceHeight = Kpl.getBlockchain().getHeight();
        }
        return (int)(adjustedWeight * (hallmarkBalance / Constants.ONE_kpl) / Constants.MAX_BALANCE_kpl);
    }
    ...
    public void blacklist(Exception cause) {
        if (cause instanceof kplException.NotCurrentlyValidException || cause instanceof BlockchainProcessor.BlockOutOfOrderException
    ...
    public void blacklist(String cause) {
        blacklistingTime = Kpl.getEpochTime();
    ...
    public JSONObject send(final JSONStreamAware request, int maxResponseSize) {
        ...
        try {
            ...
            if (useWebSocket && !webSocket.isOpen())
                useWebSocket = webSocket.startClient(URI.create("ws://" + host + ":" + getPort() + "/kpl"));
            //
            ...
            if (useWebSocket) {
                ...
                if (maxResponseSize > 0) {
                    ...
                    if (wsResponse.length() > maxResponseSize)
                        throw new kplException.kplIOException("Maximum size exceeded: " + wsResponse.length());
                ...
                //
                URL url = new URL("http://" + host + ":" + getPort() + "/kpl");
                if (communicationLoggingMask != 0)
        ...
        } catch (kplException.kplIOException e) {
            blacklist(e);
            if (connection != null) {
                connection.disconnect();
            }
    ...
    void connect() {
        lastConnectAttempt = Kpl.getEpochTime();
        try {
    ...
    boolean analyzeHallmark(final String hallmarkString) {
        ...
        try {
            ...
            for (PeerImpl peer : groupedPeers) {
                peer.adjustedWeight = Constants.MAX_BALANCE_kpl * peer.getHallmarkWeight(mostRecentDate) / totalWeight;
