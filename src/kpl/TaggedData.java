...
import kpl.db.VersionedPrunableDbTable;
import kpl.db.VersionedValuesDbTable;
...
public class TaggedData {
        ...
        protected void prune() {
            if (Constants.ENABLE_PRUNING) {
                try (Connection con = db.getConnection();
                    ...
                    int expiration = Kpl.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME;
    ...
    public static final class Tag {
        ...
        private static void add(TaggedData taggedData) {
            for (String tagValue : taggedData.getParsedTags()) {
                ...
                if (tag == null) {
                    tag = new Tag(tagValue, Kpl.getBlockchain().getHeight());
                }
                tag.count += 1;
                tagTable.insert(tag);
            }
    ...
    public TaggedData(Transaction transaction, Attachment.TaggedDataAttachment attachment) {
        this(transaction, attachment, Kpl.getBlockchain().getLastBlockTimestamp(), Kpl.getBlockchain().getHeight());
    }
    ...
    static void add(TransactionImpl transaction, Attachment.TaggedDataUpload attachment) {
        if (Kpl.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME && attachment.getData() != null) {
    ...
    static void extend(Transaction transaction, Attachment.TaggedDataExtend attachment) {
        ...
        if (Kpl.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME < timestamp.timestamp) {
            ...
            if (taggedData != null) {
                ...
                taggedData.blockTimestamp = Kpl.getBlockchain().getLastBlockTimestamp();
                taggedData.height = Kpl.getBlockchain().getHeight();
                taggedDataTable.insert(taggedData);
            }
        }
    }
