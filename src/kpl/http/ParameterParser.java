...
import static kpl.http.JSONResponses.*;
...
public final class ParameterParser {
    ...
    public static int getNumberOfConfirmations(HttpServletRequest req) throws ParameterException {
        return getInt(req, "numberOfConfirmations", 0, Kpl.getBlockchain().getHeight(), false);
    }
    ...
    public static int getHeight(HttpServletRequest req) throws ParameterException {
        ...
        if (heightValue != null) {
            try {
                ...
                if (height < 0 || height > Kpl.getBlockchain().getHeight()) {
    ...
    public static long getHoldingId(HttpServletRequest req, HoldingType holdingType) throws ParameterException {
        long holdingId = ParameterParser.getUnsignedLong(req, "holding", holdingType != HoldingType.kpl);
        if (holdingType == HoldingType.kpl && holdingId != 0) {
            throw new ParameterException(JSONResponses.incorrect("holding", "holding id should not be specified if holdingType is kpl"));
        }
        return holdingId;
    }
    ...
    public static Transaction.Builder parseTransaction(String transactionJSON, String transactionBytes, String prunableAttachmentJSON) throws ParameterException {
        ...
        if (transactionJSON != null) {
            try {
                ...
                return Kpl.newTransactionBuilder(json);
            } catch (kplException.ValidationException | RuntimeException | ParseException e) {
            ...
            try {
                ...
                return Kpl.newTransactionBuilder(bytes, prunableAttachments);
            } catch (kplException.ValidationException|RuntimeException | ParseException e) {
                Logger.logDebugMessage(e.getMessage(), e);
                JSONObject response = new JSONObject();
    ...
    public static Attachment.TaggedDataUpload getTaggedData(HttpServletRequest req) throws ParameterException, kplException.NotValidException {
