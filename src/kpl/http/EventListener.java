...
import kpl.TransactionProcessor;
 ...
 * has not been received within kpl.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by kpl.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Kpl.getIntProperty("kpl.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Kpl.getIntProperty("kpl.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Kpl.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Kpl.getTransactionProcessor();
    ...
    /** kpl event listeners */
    private final List<kplEventListener> kplEventListeners = new ArrayList<>();
     ...
     *
     * kpl event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of kpl event registrations
     ...
     *
     * @param   eventRegistrations      kpl event registrations
     * @throws  EventListenerException  Invalid kpl event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<kplEventListener> it = kplEventListeners.iterator();
                while (it.hasNext()) {
                    kplEventListener listener = it.next();
                ...
                if (addListener) {
                    kplEventListener listener = new kplEventListener(event);
                    listener.addListener();
                    kplEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            kplEventListeners.forEach(kplEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * kpl event listener
     */
    private class kplEventListener {

        /** Event handler */
        private final kplEventHandler eventHandler;

        /**
         * Create the kpl event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public kplEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the kpl listener for this event
         */
        ...
        /**
         * Remove the kpl listener for this event
         */
        ...
        /**
         * Return the hash code for this kpl event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two kpl events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof kplEventListener) &&
                    eventHandler.equals(((kplEventListener)obj).eventHandler));
        }

        /**
         * kpl listener event handler
         */
        private abstract class kplEventHandler {
            ...
            /**
             * Create the kpl event handler
             *
             * @param   eventRegistration   Event registration
             */
            public kplEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof kplEventHandler) &&
                                        owner == ((kplEventHandler)obj).owner &&
                                        accountId == ((kplEventHandler)obj).accountId &&
                                        event == ((kplEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends kplEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends kplEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends kplEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends kplEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           kpl listener event
        ...
        /**
         * Return the kpl listener event
         *
         * @return                  kpl listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
