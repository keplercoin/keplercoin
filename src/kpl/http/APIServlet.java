...
import static kpl.http.JSONResponses.ERROR_DISABLED;
import static kpl.http.JSONResponses.ERROR_INCORRECT_REQUEST;
import static kpl.http.JSONResponses.ERROR_NOT_ALLOWED;
import static kpl.http.JSONResponses.POST_REQUIRED;
import static kpl.http.JSONResponses.REQUIRED_BLOCK_NOT_FOUND;
import static kpl.http.JSONResponses.REQUIRED_LAST_BLOCK_NOT_FOUND;
...
public final class APIServlet extends HttpServlet {
    ...
    public abstract static class APIRequestHandler {
        ...
        protected abstract JSONStreamAware processRequest(HttpServletRequest request) throws kplException;
    ...
    private static final boolean enforcePost = Kpl.getBooleanProperty("kpl.apiServerEnforcePOST");
    ...
    static {
        ...
        map.put("appLanguage", AppLanguage.instance); //by xiaoc
        ...
        API.disabledAPIs.forEach(api -> {
            ...
            if (handler == null) {
                throw new RuntimeException("Invalid API in kpl.disabledAPIs: " + api);
            }
            ...
            try {
                ...
                try {
                    try {
                        ...
                        if (requireBlockId != 0 && !Kpl.getBlockchain().hasBlock(requireBlockId)) {
                            response = REQUIRED_BLOCK_NOT_FOUND;
                            return;
                        }
                        if (requireLastBlockId != 0 && requireLastBlockId != Kpl.getBlockchain().getLastBlock().getId()) {
                        ...
                        if (requireLastBlockId == 0 && requireBlockId != 0 && response instanceof JSONObject) {
                            ((JSONObject) response).put("lastBlock", Kpl.getBlockchain().getLastBlock().getStringId());
                        }
                    } finally {
                        if (apiRequestHandler.startDbTransaction()) {
