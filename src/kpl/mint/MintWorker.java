...
public class MintWorker {
    ...
    private void mint() {
        String currencyCode = Convert.emptyToNull(Kpl.getStringProperty("kpl.mint.currencyCode"));
        if (currencyCode == null) {
            throw new IllegalArgumentException("kpl.mint.currencyCode not specified");
        }
        String secretPhrase = Convert.emptyToNull(Kpl.getStringProperty("kpl.mint.secretPhrase", null, true));
        if (secretPhrase == null) {
            throw new IllegalArgumentException("kpl.mint.secretPhrase not specified");
        }
        boolean isSubmitted = Kpl.getBooleanProperty("kpl.mint.isSubmitted");
        boolean isStopOnError = Kpl.getBooleanProperty("kpl.mint.stopOnError");
        ...
        String unitsStr = Kpl.getStringProperty("kpl.mint.unitsPerMint");
        double wholeUnits = 1;
        ...
        long initialNonce = Kpl.getIntProperty("kpl.mint.initialNonce");
        if (initialNonce == 0) {
            initialNonce = new Random().nextLong();
        }
        int threadPoolSize = Kpl.getIntProperty("kpl.mint.threadPoolSize");
        if (threadPoolSize == 0) {
    ...
    private JSONObject mintImpl(String secretPhrase, long accountId, long units, long currencyId, byte algorithm,
        ...
        if (isSubmitted) {
            ...
            response.put("message", "kpl.mint.isSubmitted=false therefore currency mint transaction is not submitted");
        }
        return response;
    }
    ...
    private JSONObject currencyMint(String secretPhrase, long currencyId, long nonce, long units, long counter) {
        ...
        Transaction.Builder builder = Kpl.newTransactionBuilder(Crypto.getPublicKey(secretPhrase), 0, Constants.ONE_kpl,
    ...
    private JSONObject getJsonResponse(Map<String, String> params) {
        ...
        String host = Convert.emptyToNull(Kpl.getStringProperty("kpl.mint.serverAddress"));
        if (host == null) {
            try {
        ...
        boolean useHttps = Kpl.getBooleanProperty("kpl.mint.useHttps");
        if (useHttps) {
            protocol = "https";
            HttpsURLConnection.setDefaultSSLSocketFactory(TrustAllSSLProvider.getSslSocketFactory());
            HttpsURLConnection.setDefaultHostnameVerifier(TrustAllSSLProvider.getHostNameVerifier());
        }
        int port = Constants.isTestnet ? API.TESTNET_API_PORT : Kpl.getIntProperty("kpl.apiServerPort");
        String urlParams = getUrlParams(params);
        URL url;
        try {
            url = new URL(protocol, host, port, "/kpl?" + urlParams);
