 ...
 * <p>
 * kpl, ASSET and CURRENCY balances can be monitored.  If a balance falls below the threshold, a transaction
...
public final class FundingMonitor {
    ...
    private static final int MAX_MONITORS = Kpl.getIntProperty("kpl.maxNumberOfMonitors");
     ...
     * @param   holdingId           Asset or Currency identifier, ignored for kpl monitor
    ...
    private FundingMonitor(HoldingType holdingType, long holdingId, String property,
        ...
        this.holdingId = (holdingType != HoldingType.kpl ? holdingId : 0);
    ...
    public static boolean startMonitor(HoldingType holdingType, long holdingId, String property,
        ...
        // instead of during KRS initialization so we don't start the monitor thread if it
        // won't be used.
        //
        init();
        ...
        synchronized(monitors) {
            ...
            while (monitorIt.hasNext()) {
                ...
                if (monitor.holdingType == holdingType && monitor.property.equals(property) &&
                        (holdingType == HoldingType.kpl || monitor.holdingId == holdingId) &&
    ...
    private static synchronized void init() {
        ...
        try {
            ...
            Kpl.getBlockchainProcessor().addListener(new BlockEventHandler(), BlockchainProcessor.Event.BLOCK_PUSHED);
            //
            // All done
            //
            started = true;
                ...
                while (true) {
                    ...
                    while ((monitoredAccount = pendingEvents.poll()) != null) {
                        try {
                            ...
                            if (Kpl.getBlockchain().getHeight() - monitoredAccount.height < monitoredAccount.interval) {
                                ...
                                switch (monitoredAccount.monitor.holdingType) {
                                    case kpl:
                                        processkplEvent(monitoredAccount, targetAccount, fundingAccount);
                                        break;
                                    case ASSET:
     ...
     * @throws  kplException                Unable to create transaction
     */
    private static void processkplEvent(MonitoredAccount monitoredAccount, Account targetAccount, Account fundingAccount)
                                            throws kplException {
        FundingMonitor monitor = monitoredAccount.monitor;
        if (targetAccount.getBalanceNQT() < monitoredAccount.threshold) {
            Transaction.Builder builder = Kpl.newTransactionBuilder(monitor.publicKey,
                    monitoredAccount.amount, 0, (short)1440, Attachment.ORDINARY_PAYMENT);
            builder.recipientId(monitoredAccount.accountId)
                   .timestamp(Kpl.getBlockchain().getLastBlockTimestamp());
            ...
            if (Math.addExact(monitoredAccount.amount, transaction.getFeeNQT()) > fundingAccount.getUnconfirmedBalanceNQT()) {
                ...
                monitoredAccount.height = Kpl.getBlockchain().getHeight();
                Logger.logDebugMessage(String.format("kpl funding transaction %s for %f kpl submitted from %s to %s",
                        transaction.getStringId(), (double)monitoredAccount.amount / Constants.ONE_kpl,
    ...
    private static final class AccountEventHandler implements Listener<Account> {
        ...
        public void notify(Account account) {
            ...
            //
            // Check the kpl balance for monitored accounts
            //
            synchronized(monitors) {
                List<MonitoredAccount> accountList = accounts.get(account.getId());
                if (accountList != null) {
                    accountList.forEach((maccount) -> {
                       if (maccount.monitor.holdingType == HoldingType.kpl && balance < maccount.threshold &&
