...
import kpl.util.ReadWriteUpdateLock;
 ...
 * provided as a convenience for use in kplDbVersion when creating a new index
 * after the database has been created.
 *
 * FTL_DROP_INDEX is called to drop a fulltext index for a table.  It is
 * provided as a convenience for use in kplDbVersion when dropping a table
 * after the database has been created.
 *
 ...
 *   COLUMNS - the primary key columns (String[]) - this is always DB_ID for KRS
...
public class FullTextTrigger implements Trigger, TransactionalDb.TransactionCallback {
    ...
    /**
     * This method is called by KRS initialization to indicate KRS is active.
     *
     * This is required since database triggers will be initialized when the database
     * is opened outside the KRS environment (for example, from the H2 console)
     *
     ...
     *
     * This method should be called from kplDbVersion when performing the database version update
     * that enables KRS fulltext search support
     */
    public static void init() {
        ...
        try (Connection conn = Db.db.getConnection();
            ...
            if (triggersExist && alreadyInitialized) {
                Logger.logInfoMessage("KRS fulltext support is already initialized");
                return;
            }
            //
            ...
            Logger.logInfoMessage("KRS fulltext schema created");
            //
            ...
            Logger.logInfoMessage("KRS fulltext aliases created");
        } catch (SQLException exc) {
            Logger.logErrorMessage("Unable to initialize KRS fulltext search support", exc);
    ...
    public static void createIndex(Connection conn, String schema, String table, String columnList)
        ...
        if (trigger == null) {
            Logger.logErrorMessage("KRS fulltext trigger for table " + tableName + " was not initialized");
        } else {
            try {
                trigger.reindexTable(conn);
    ...
    public void init(Connection conn, String schema, String trigger, String table, boolean before, int type)
                                    throws SQLException {
        //
        // Ignore the trigger if KRS is not active or this is a temporary table copy
        //
        ...
        try (Statement stmt = conn.createStatement()) {
            ...
            //
            // KRS tables use DB_ID as the primary index
            //
