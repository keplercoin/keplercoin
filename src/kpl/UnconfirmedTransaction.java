...
class UnconfirmedTransaction implements Transaction {
    ...
    void save(Connection con) throws SQLException {
        try (PreparedStatement pstmt = con.prepareStatement("INSERT INTO unconfirmed_transaction (id, transaction_height, "
                ...
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?)")) {
            int i = 0;
            pstmt.setLong(++i, transaction.getId()); //id
            pstmt.setInt(++i, transaction.getHeight()); //transaction_height
            pstmt.setLong(++i, feePerByte); //fee_per_byte
            pstmt.setInt(++i, transaction.getExpiration()); //expiration
            pstmt.setBytes(++i, transaction.bytes()); //transaction_bytes
            JSONObject prunableJSON = transaction.getPrunableAttachmentJSON(); //prunable_json
            if (prunableJSON != null) {
                pstmt.setString(++i, prunableJSON.toJSONString()); //arrival_timestamp
            } else {
                pstmt.setNull(++i, Types.VARCHAR);
            }
            pstmt.setLong(++i, arrivalTimestamp);
            pstmt.setInt(++i, Kpl.getBlockchain().getHeight()); //height
            pstmt.executeUpdate();
        }
    }
