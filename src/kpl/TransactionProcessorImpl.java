...
final class TransactionProcessorImpl implements TransactionProcessor {
    ...
    private static final boolean enableTransactionRebroadcasting = Kpl.getBooleanProperty("kpl.enableTransactionRebroadcasting");
    private static final boolean testUnconfirmedTransactions = Kpl.getBooleanProperty("kpl.testUnconfirmedTransactions");
    private static final int maxUnconfirmedTransactions;
    static {
        int n = Kpl.getIntProperty("kpl.maxUnconfirmedTransactions");
    ...
    private final Runnable removeUnconfirmedTransactionsThread = () -> {
        ...
        try {
            try {
                if (Kpl.getBlockchainProcessor().isDownloading() && ! testUnconfirmedTransactions) {
                    return;
                }
                ...
                try (DbIterator<UnconfirmedTransaction> iterator = unconfirmedTransactionTable.getManyBy(
                        new DbClause.IntClause("expiration", DbClause.Op.LT, Kpl.getEpochTime()), 0, -1, "")) {
    ...
    @Override
    public void broadcast(Transaction transaction) throws kplException.ValidationException {
        BlockchainImpl.getInstance().writeLock();
        try {
    ...
    @Override
    public void processPeerTransactions(JSONObject request) throws kplException.ValidationException {
    ...
    void processWaitingTransactions() {
        ...
        try {
            if (waitingTransactions.size() > 0) {
                int currentTime = Kpl.getEpochTime();
                ...
                while (iterator.hasNext()) {
                    ...
                    } catch (kplException.ExistingTransactionException e) {
                        iterator.remove();
                    } catch (kplException.NotCurrentlyValidException e) {
    ...
    private void processPeerTransactions(JSONArray transactionsData) throws kplException.NotValidException {
        ...
        for (Object transactionData : transactionsData) {
            ...
            } catch (kplException.NotCurrentlyValidException ignore) {
            } catch (kplException.ValidationException|RuntimeException e) {
        ...
        if (!exceptions.isEmpty()) {
            throw new kplException.NotValidException("Peer sends invalid transactions: " + exceptions.toString());
        }
    }

    private void processTransaction(UnconfirmedTransaction unconfirmedTransaction) throws kplException.ValidationException {
        TransactionImpl transaction = unconfirmedTransaction.getTransaction();
        int curTime = Kpl.getEpochTime();
        if (transaction.getTimestamp() > curTime + Constants.MAX_TIMEDRIFT || transaction.getExpiration() < curTime) {
            throw new kplException.NotCurrentlyValidException("Invalid transaction timestamp");
        }
        if (transaction.getVersion() < 1) {
            throw new kplException.NotValidException("Invalid transaction version");
        }
        if (transaction.getId() == 0L) {
            throw new kplException.NotValidException("Invalid transaction id 0");
        }
        ...
        try {
            try {
                ...
                if (getUnconfirmedTransaction(transaction.getDbKey()) != null || TransactionDb.hasTransaction(transaction.getId())) {
                    throw new kplException.ExistingTransactionException("Transaction already processed");
                }

                if (! transaction.verifySignature()) {
                    if (Account.getAccount(transaction.getSenderId()) != null) {
                        throw new kplException.NotValidException("Transaction signature verification failed");
                    } else {
                        throw new kplException.NotCurrentlyValidException("Unknown transaction sender");
                    }
                }

                if (! transaction.applyUnconfirmed()) {
                    throw new kplException.InsufficientBalanceException("Insufficient balance");
                }

                if (transaction.isUnconfirmedDuplicate(unconfirmedDuplicates)) {
                    throw new kplException.NotCurrentlyValidException("Duplicate unconfirmed transaction");
                }
     ...
     * @throws  kplException.NotValidException    Transaction is not valid
     */
    @Override
    public List<Transaction> restorePrunableData(JSONArray transactions) throws kplException.NotValidException {
