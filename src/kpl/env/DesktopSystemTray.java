...
public class DesktopSystemTray {
    ...
    void createAndShowGUI() {
        ...
        imageIcon = new ImageIcon("html/ui/img/KPL-icon-32x32.png", "tray icon");
        ...
        if (!Kpl.isDesktopApplicationEnabled()) {
        ...
        showDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("kpldesktop.DesktopApplication").getMethod("launch").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("kpldesktop.DesktopApplication failed to launch", exception);
            }
        });

        refreshDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("kpldesktop.DesktopApplication").getMethod("refresh").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("kpldesktop.DesktopApplication failed to refresh", exception);
            }
        });
    ...
    private void displayStatus() {
        ...
        addDataRow(statusPanel, "Application", Kpl.APPLICATION);
        addDataRow(statusPanel, "Version", Kpl.VERSION);
        ...
        addDataRow(statusPanel, "User folder", String.valueOf(Paths.get(Kpl.getUserHomeDir()).toAbsolutePath()));
        ...
        if (lastBlock != null) {
            ...
            addDataRow(statusPanel, "Seconds passed", String.valueOf(Kpl.getEpochTime() - lastBlock.getTimestamp()));
        ...
        addDataRow(statusPanel, "Process id", Kpl.getProcessId());
        addEmptyRow(statusPanel);
        ...
        if (statusDialog == null || !statusDialog.isVisible()) {
            ...
            statusDialog = pane.createDialog(wrapper, "kpl Server Status");
