...
/**
 * Compact and reorganize the KRS database.  The KRS application must not be
 * running.
 *
 ...
 *
 *   java -cp "classes;lib/*;conf" -Dkpl.runtime.mode=desktop CompactDatabase
 */
public class CompactDatabase {

    /**
     * Compact the KRS database
     *
     * @param   args                Command line arguments
     */
    public static void main(String[] args) {
        //
        // Initialize kpl properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "kpl.testDb" : "kpl.db";
        String dbType = Kpl.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Kpl.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Kpl.getDbDir(Kpl.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Kpl.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Kpl.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Kpl.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "kpl.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "kpl.mv.db");
            if (!dbFile.exists()) {
                Logger.logErrorMessage("KRS database not found");
                return 1;
            }
        }
