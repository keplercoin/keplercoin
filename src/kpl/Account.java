...
import kpl.AccountLedger.LedgerHolding;
...
import kpl.db.VersionedPersistentDbTable;
...
public final class Account {
    ...
    public static final class PublicKey {
        ...
        private PublicKey(long accountId, byte[] publicKey) {
            ...
            this.height = Kpl.getBlockchain().getHeight();
        }
    ...
    static {
        ...
        Kpl.getBlockchainProcessor().addListener(block -> {
        ...
        if (publicKeyCache != null) {
            ...
            Kpl.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public long getEffectiveBalancekpl() {
        return getEffectiveBalancekpl(Kpl.getBlockchain().getHeight());
    }

    //得到有效余额
    public long getEffectiveBalancekpl(int height) {
        if (height >= Constants.TRANSPARENT_FORGING_BLOCK_6) {
        ...
        if (height < Constants.TRANSPARENT_FORGING_BLOCK_3) {
            if (Arrays.binarySearch(Genesis.GENESIS_RECIPIENTS, id) >= 0) {
                return balanceNQT / Constants.ONE_kpl;
            }
            long receivedInLastBlock = 0;
            for (Transaction transaction : Kpl.getBlockchain().getBlockAtHeight(height).getTransactions()) {
                if (id == transaction.getRecipientId()) {
                    receivedInLastBlock += transaction.getAmountNQT();
                }
            }
            return (balanceNQT - receivedInLastBlock) / Constants.ONE_kpl;
        }
        ...
        try {
            ...
            return (height > Constants.SHUFFLING_BLOCK && effectiveBalanceNQT < Constants.MIN_FORGING_BALANCE_NQT) ? 0 : effectiveBalanceNQT / Constants.ONE_kpl;
        } finally {
    ...
    public long getGuaranteedBalanceNQT() {
        return getGuaranteedBalanceNQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, Kpl.getBlockchain().getHeight());
    }
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations, final int currentHeight) {
        ...
        try {
            ...
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < Kpl.getBlockchainProcessor().getMinRollbackHeight()
                    || height > Kpl.getBlockchain().getHeight()) {
    ...
    void leaseEffectiveBalance(long lesseeId, int period) {
        int height = Kpl.getBlockchain().getHeight();
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = Kpl.getBlockchain().getHeight();
            return true;
        }
        ...
        if (AccountLedger.mustLogEntry(this.id, false)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.kpl_BALANCE, null, feeNQT, this.balanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.kpl_BALANCE, null, amountNQT, this.balanceNQT));
            }
        }
    }
    ...
    void addToUnconfirmedBalanceNQT(LedgerEvent event, long eventId, long amountNQT, long feeNQT) {
        ...
        if (AccountLedger.mustLogEntry(this.id, true)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_kpl_BALANCE, null, feeNQT, this.unconfirmedBalanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_kpl_BALANCE, null, amountNQT, this.unconfirmedBalanceNQT));
            }
        }
    }
