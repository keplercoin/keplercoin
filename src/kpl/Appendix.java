...
public interface Appendix {
    ...
    interface Prunable {
        ...
        default boolean shouldLoadPrunable(Transaction transaction, boolean includeExpiredPrunable) {
            return Kpl.getEpochTime() - transaction.getTimestamp() <
    ...
    abstract class AbstractAppendix implements Appendix {
        ...
        boolean verifyVersion(byte transactionVersion) {
            return transactionVersion == 0 ? version == 0 : (Kpl.getBlockchain().getHeight() < Constants.FXT_BLOCK ? version > 0 : version == 1);
        }
        ...
        abstract void validate(Transaction transaction) throws kplException.ValidationException;

        void validateAtFinish(Transaction transaction) throws kplException.ValidationException {
    ...
    class Message extends AbstractAppendix {
        ...
        private static final Fee MESSAGE_FEE = new Fee.SizeBasedFee(0, Constants.ONE_kpl, 32) {
            @Override
        ...
        Message(ByteBuffer buffer, byte transactionVersion) throws kplException.NotValidException {
            ...
            if (messageLength > 1000) {
                throw new kplException.NotValidException("Invalid arbitrary message length: " + messageLength);
            }
            this.message = new byte[messageLength];
            buffer.get(this.message);
            if (isText && !Arrays.equals(message, Convert.toBytes(Convert.toString(message)))) {
                throw new kplException.NotValidException("Message is not UTF-8 text");
            }
        }
        ...
        @Override
        void validate(Transaction transaction) throws kplException.ValidationException {
            if (Kpl.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && message.length > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
                throw new kplException.NotValidException("Invalid arbitrary message length: " + message.length);
            }
        }
    ...
    class PrunablePlainMessage extends Appendix.AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_MESSAGE_FEE = new Fee.SizeBasedFee(Constants.ONE_kpl/10) {
            @Override
        ...
        void validate(Transaction transaction) throws kplException.ValidationException {
            if (transaction.getMessage() != null) {
                throw new kplException.NotValidException("Cannot have both message and prunable message attachments");
            }
            byte[] msg = getMessage();
            if (msg != null && msg.length > Constants.MAX_PRUNABLE_MESSAGE_LENGTH) {
                throw new kplException.NotValidException("Invalid prunable message length: " + msg.length);
            }
            if (msg == null && Kpl.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                throw new kplException.NotCurrentlyValidException("Message has been pruned prematurely");
            }
        }

        @Override
        void apply(Transaction transaction, Account senderAccount, Account recipientAccount) {
            if (Kpl.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME) {
    ...
    abstract class AbstractEncryptedMessage extends AbstractAppendix {
        ...
        private static final Fee ENCRYPTED_MESSAGE_FEE = new Fee.SizeBasedFee(Constants.ONE_kpl, Constants.ONE_kpl, 32) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendage) {
        ...
        private AbstractEncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws kplException.NotValidException {
            super(buffer, transactionVersion);
        ...
        @Override
        void validate(Transaction transaction) throws kplException.ValidationException {
            if (Kpl.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && getEncryptedDataLength() > Constants.MAX_ENCRYPTED_MESSAGE_LENGTH) {
                throw new kplException.NotValidException("Max encrypted message length exceeded");
            }
            if (encryptedData != null) {
                if ((encryptedData.getNonce().length != 32 && encryptedData.getData().length > 0)
                    ...
                    throw new kplException.NotValidException("Invalid nonce length " + encryptedData.getNonce().length);
                }
            }
            if ((getVersion() != 2 && !isCompressed) || (getVersion() == 2 && isCompressed)) {
                throw new kplException.NotValidException("Version mismatch - version " + getVersion() + ", isCompressed " + isCompressed);
            }
        }
    ...
    class PrunableEncryptedMessage extends AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_ENCRYPTED_DATA_FEE = new Fee.SizeBasedFee(Constants.ONE_kpl/10) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendix) {
                return appendix.getFullSize();
            }
        ...
        void validate(Transaction transaction) throws kplException.ValidationException {
            if (transaction.getEncryptedMessage() != null) {
                throw new kplException.NotValidException("Cannot have both encrypted and prunable encrypted message attachments");
            }
            EncryptedData ed = getEncryptedData();
            if (ed == null && Kpl.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                throw new kplException.NotCurrentlyValidException("Encrypted message has been pruned prematurely");
            }
            if (ed != null) {
                if (ed.getData().length > Constants.MAX_PRUNABLE_ENCRYPTED_MESSAGE_LENGTH) {
                    throw new kplException.NotValidException(String.format("Message length %d exceeds max prunable encrypted message length %d",
                ...
                if ((ed.getNonce().length != 32 && ed.getData().length > 0)
                    ...
                    throw new kplException.NotValidException("Invalid nonce length " + ed.getNonce().length);
                }
            }
            if (transaction.getRecipientId() == 0) {
                throw new kplException.NotValidException("Encrypted messages cannot be attached to transactions with no recipient");
            }
        }
    ...
    final class UnencryptedPrunableEncryptedMessage extends PrunableEncryptedMessage implements Encryptable {
        ...
        void putMyBytes(ByteBuffer buffer) {
            if (getEncryptedData() == null) {
                throw new kplException.NotYetEncryptedException("Prunable encrypted message not yet encrypted");
            }
    ...
    class EncryptedMessage extends AbstractEncryptedMessage {
        ...
        EncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws kplException.NotValidException {
            super(buffer, transactionVersion);
        }
    ...
    final class UnencryptedEncryptedMessage extends EncryptedMessage implements Encryptable {
        ...
        void putMyBytes(ByteBuffer buffer) {
            if (getEncryptedData() == null) {
                throw new kplException.NotYetEncryptedException("Message not yet encrypted");
            }
            super.putMyBytes(buffer);
        }
    ...
    class EncryptToSelfMessage extends AbstractEncryptedMessage {
        ...
        EncryptToSelfMessage(ByteBuffer buffer, byte transactionVersion) throws kplException.NotValidException {
            super(buffer, transactionVersion);
        }
    ...
    final class PublicKeyAnnouncement extends AbstractAppendix {
        ...
        void validate(Transaction transaction) throws kplException.ValidationException {
            if (transaction.getRecipientId() == 0) {
                throw new kplException.NotValidException("PublicKeyAnnouncement cannot be attached to transactions with no recipient");
            }
            if (!Crypto.isCanonicalPublicKey(publicKey)) {
                throw new kplException.NotValidException("Invalid recipient public key: " + Convert.toHexString(publicKey));
            }
            long recipientId = transaction.getRecipientId();
            if (Account.getId(this.publicKey) != recipientId) {
                throw new kplException.NotValidException("Announced public key does not match recipient accountId");
            }
            ...
            if (recipientPublicKey != null && ! Arrays.equals(publicKey, recipientPublicKey)) {
                throw new kplException.NotCurrentlyValidException("A different public key for this account has already been announced");
            }
        }
    ...
    final class Phasing extends AbstractAppendix {
        ...
        private static final Fee PHASING_FEE = (transaction, appendage) -> {
            ...
            if (!phasing.params.getVoteWeighting().isBalanceIndependent()) {
                fee += 20 * Constants.ONE_kpl;
            } else {
                fee += Constants.ONE_kpl;
            }
            if (phasing.hashedSecret.length > 0) {
                fee += (1 + (phasing.hashedSecret.length - 1) / 32) * Constants.ONE_kpl;
            }
            fee += Constants.ONE_kpl * phasing.linkedFullHashes.length;
            return fee;
        };
        ...
        void validate(Transaction transaction) throws kplException.ValidationException {
            params.validate();
            int currentHeight = Kpl.getBlockchain().getHeight();
            if (params.getVoteWeighting().getVotingModel() == VoteWeighting.VotingModel.TRANSACTION) {
                if (linkedFullHashes.length == 0 || linkedFullHashes.length > Constants.MAX_PHASING_LINKED_TRANSACTIONS) {
                    throw new kplException.NotValidException("Invalid number of linkedFullHashes " + linkedFullHashes.length);
                }
                Set<Long> linkedTransactionIds = new HashSet<>(linkedFullHashes.length);
                for (byte[] hash : linkedFullHashes) {
                    if (Convert.emptyToNull(hash) == null || hash.length != 32) {
                        throw new kplException.NotValidException("Invalid linkedFullHash " + Convert.toHexString(hash));
                    }
                    if (Kpl.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK) {
                        if (!linkedTransactionIds.add(Convert.fullHashToId(hash))) {
                            throw new kplException.NotValidException("Duplicate linked transaction ids");
                        }
                    }
                    ...
                    if (linkedTransaction != null) {
                        if (transaction.getTimestamp() - linkedTransaction.getTimestamp() > Constants.MAX_REFERENCED_TRANSACTION_TIMESPAN) {
                            throw new kplException.NotValidException("Linked transaction cannot be more than 60 days older than the phased transaction");
                        }
                        if (linkedTransaction.getPhasing() != null) {
                            throw new kplException.NotCurrentlyValidException("Cannot link to an already existing phased transaction");
                        }
                    }
                }
                if (params.getQuorum() > linkedFullHashes.length) {
                    throw new kplException.NotValidException("Quorum of " + params.getQuorum() + " cannot be achieved in by-transaction voting with "
                            + linkedFullHashes.length + " linked full hashes only");
                }
            } else {
                if (linkedFullHashes.length != 0) {
                    throw new kplException.NotValidException("LinkedFullHashes can only be used with VotingModel.TRANSACTION");
                }
            }

            if (params.getVoteWeighting().getVotingModel() == VoteWeighting.VotingModel.HASH) {
                if (params.getQuorum() != 1) {
                    throw new kplException.NotValidException("Quorum must be 1 for by-hash voting");
                }
                if (hashedSecret.length == 0 || hashedSecret.length > Byte.MAX_VALUE) {
                    throw new kplException.NotValidException("Invalid hashedSecret " + Convert.toHexString(hashedSecret));
                }
                if (PhasingPoll.getHashFunction(algorithm) == null) {
                    throw new kplException.NotValidException("Invalid hashedSecretAlgorithm " + algorithm);
                }
            } else {
                if (hashedSecret.length != 0) {
                    throw new kplException.NotValidException("HashedSecret can only be used with VotingModel.HASH");
                }
                if (algorithm != 0) {
                    throw new kplException.NotValidException("HashedSecretAlgorithm can only be used with VotingModel.HASH");
                }
            }
            ...
            if (finishHeight <= currentHeight + (params.getVoteWeighting().acceptsVotes() ? 2 : 1)
                ...
                throw new kplException.NotCurrentlyValidException("Invalid finish height " + finishHeight);
            }
        }
        ...
        void countVotes(TransactionImpl transaction) {
            if (Kpl.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && PhasingPoll.getResult(transaction.getId()) != null) {
                return;
            }
            ...
            if (result >= poll.getQuorum()) {
                if (!transaction.attachmentIsDuplicate(duplicates, false)) {
                    try {
                        release(transaction);
                        poll.finish(result);
                        Logger.logDebugMessage("Early finish of transaction " + transaction.getStringId() + " at height " + Kpl.getBlockchain().getHeight());
                    } catch (RuntimeException e) {
                        Logger.logErrorMessage("Failed to release phased transaction " + transaction.getJSONObject().toJSONString(), e);
                    }
                } else {
                    Logger.logDebugMessage("At height " + Kpl.getBlockchain().getHeight() + " phased transaction " + transaction.getStringId()
