...
public final class Shuffler {
    ...
    private static final int MAX_SHUFFLERS = Kpl.getIntProperty("kpl.maxNumberOfShufflers");
    ...
    private static void scheduleExpiration(Shuffling shuffling) {
        int expirationHeight = Kpl.getBlockchain().getHeight() + 720;
    ...
    private volatile kplException.NotCurrentlyValidException failureCause;
    ...
    public kplException.NotCurrentlyValidException getFailureCause() {
        return failureCause;
    }
    ...
    private void submitTransaction(Attachment.ShufflingAttachment attachment) {
        ...
        try {
            Transaction.Builder builder = Kpl.newTransactionBuilder(Crypto.getPublicKey(secretPhrase), 0, 0,
                    (short) 1440, attachment);
            builder.timestamp(Kpl.getBlockchain().getLastBlockTimestamp());
            ...
            if (participantAccount == null || transaction.getFeeNQT() > participantAccount.getUnconfirmedBalanceNQT()) {
                ...
                failureCause = new kplException.NotCurrentlyValidException("Insufficient balance");
    ...
    public static class ShufflerException extends kplException {
