...
var krs = (function(krs, $, undefined) {
    ...
    modal.on("show.bs.modal", function() {
		...
		if ($visible_modal.length) {
			if ($visible_modal.hasClass("locked")) {
				...
				krs.unlockForm($visible_modal, $btn, true);
			} else {
		...
		krs.initAdvancedModalFormValues($(this));
	...
	modal.on("shown.bs.modal", function() {
		...
		krs.showedFormWarning = false; //maybe not the best place... we assume forms are only in modals?
	});
	...
	modal.on("hidden.bs.modal", function() {
		...
		krs.showedFormWarning = false;
	});

	krs.showModalError = function(errorMessage, $modal) {
