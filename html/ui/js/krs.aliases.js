...
var krs = (function (krs, $, undefined) {
    krs.pages.aliases = function () {
        var alias_count;
        krs.sendRequest("getAliasCount+", {"account": krs.account}, function (response) {
        ...
        krs.sendRequest("getAliases+", {
            ...
            if (response.aliases && response.aliases.length) {
                if (response.aliases.length > krs.itemsPerPage) {
                ...
                krs.dataLoadFinished(aliasesTable);
    ...
    krs.setup.aliases = function () {
        var options = {
            "id": 'sidebar_aliases',
            "titleHTML": '<i class="fa fa-bookmark"></i> <span data-i18n="aliases">Aliases</span>',
            "page": 'aliases',
            "desiredPosition": 100,
            "depends": { tags: [ krs.constants.API_TAGS.ALIASES ] }
        };
        krs.addSimpleSidebarMenuItem(options);
    };
    ...
    krs.forms.sellAlias = function ($modal) {
        ...
        if (data.modal == "cancel_alias_sale") {
            ...
            if (data.recipient == krs.constants.GENESIS_RS) {
                if (!data.priceKPL || data.priceKPL == "0") {
                    return {
                        "error": $.t("error_not_specified", {
    ...
    krs.forms.sellAliasComplete = function (response, data) {
        if (response.alreadyProcessed) {
            return;
        }
        data.aliasName = String(data.aliasName).escapeHTML();
        if (data.priceNQT == "0") {
            if (data.recipient == krs.account) {
                $.growl(
                    $.t("cancelling_sale", {
            ...
            $.growl(
                $.t("success_alias_sell", {
                    ...
                    "price": krs.convertToKPL(data.priceNQT).escapeHTML()
                }), {
                    "type": "success"
                }
            );
    ...
    krs.forms.deleteAlias = function ($modal) {
        var data = krs.getFormData($modal.find("form:first"));
    ...
    krs.forms.deleteAliasComplete = function (response, data) {
        if (response.alreadyProcessed) {
            return;
        }
        data.aliasName = String(data.aliasName).escapeHTML();
    ...
    $("#sell_alias_to_specific_account, #sell_alias_to_anyone").on("click", function (e) {
        ...
        if ($(this).attr("id") == "sell_alias_to_anyone") {
            $modal.find("input[name=recipient]").val(krs.constants.GENESIS_RS);
    ...
    $("#buy_alias_modal").on("show.bs.modal", function (e) {
        ...
        krs.sendRequest("getAlias", {
            ...
            if (response.errorCode) {
                ...
                } else if (typeof response.buyer != "undefined" && response.buyer != krs.account) {
                    e.preventDefault();
    ...
    krs.forms.buyAliasError = function () {
        $("#buy_alias_modal").find("input[name=priceKPL]").prop("readonly", false);
    };

    krs.forms.buyAliasComplete = function (response, data) {
        if (response.alreadyProcessed) {
            return;
        }
        data.aliasName = String(data.aliasName).escapeHTML();
        $.growl(
            $.t("success_alias_buy", {
                "alias_name": data.aliasName,
                "price": krs.convertToKPL(data.amountNQT).escapeHTML()
            }), {
                "type": "success"
            }
        );
        ...
        if (alias) {
            ...
            krs.sendRequest("getAlias", {
                ...
                if (response.errorCode) {
                    ...
                    } else if ((aliasURI = /acct:(.*)@KPL/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
    ...
    krs.incoming.aliases = function (transactions) {
        if (krs.hasTransactionUpdates(transactions)) {
            krs.loadPage("aliases");
        }
    };

    krs.forms.setAlias = function ($modal) {
        var data = krs.getFormData($modal.find("form:first"));

        data.aliasURI = $.trim(data.aliasURI).toLowerCase();

        if (data.type == "account") {
            if (!(/acct:(.*)@KPL/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
                if (/^(KPL\-)/i.test(data.aliasURI)) {
                    ...
                    if (!address.set(data.aliasURI)) {
                        ...
                        data.aliasURI = "acct:" + data.aliasURI + "@KPL";
                    }
    ...
    function setAliasType(type, uri) {
        ...
        if (type == "uri") {
            ...
            registerAliasUri.val("").mask("KPL-****-****-****-*****");

            if (uri) {
                var match = uri.match(/acct:(.*)@KPL/i);
                if (!match) {
                ...
                } else if (!/^KPL\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
                    uri = krs.accountRS;
                }
                ...
                registerAliasUri.val(krs.accountRS);
            }
            ...
            krs.sendRequest("getAlias", {
                ...
                if (!response.errorCode) {
                    if ("priceNQT" in response) {
                        ...
                        message = "<a href='#' class='show_account_modal_action' data-user='" + krs.getAccountFormatted(response, "account") + "'>" + $.t("view_owner_info_q") + "</a>";
                    }
    ...
    krs.forms.setAliasComplete = function (response, data) {
        if (response.alreadyProcessed) {
            return;
        }
        data.aliasName = String(data.aliasName).escapeHTML();
        if (krs.currentPage == "aliases") {
            var aliasesTable = $("#aliases_table");
        ...
        krs.sendRequest("getAlias", {
            ...
            if (response.errorCode) {
                ...
                var data = {
                    ...
                    "last_updated": krs.formatTimestamp(response.timestamp),
                ...
                $("#alias_info_table").find("tbody").append(krs.createInfoTable(data));
