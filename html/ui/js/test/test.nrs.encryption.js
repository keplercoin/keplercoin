QUnit.module("krs.encryption");

QUnit.test("generatePublicKey", function (assert) {
    assert.throws(function() { krs.generatePublicKey("") }, "Can't generate public key without the user's password.", "empty.public.key");
    assert.equal(krs.generatePublicKey("12345678"), "a65ae5bc3cdaa9a0dd66f2a87459bbf663140060e99ae5d4dfe4dbef561fdd37", "public.key");
    assert.equal(krs.generatePublicKey("hope peace happen touch easy pretend worthless talk them indeed wheel state"), "112e0c5748b5ea610a44a09b1ad0d2bddc945a6ef5edc7551b80576249ba585b", "public.key");
});

QUnit.test("getPublicKey", function (assert) {
    var publicKey1 = krs.getPublicKey(converters.stringToHexString("12345678"));
...
QUnit.test("getAccountIdFromPublicKey", function (assert) {
    assert.equal(krs.getAccountIdFromPublicKey("112e0c5748b5ea610a44a09b1ad0d2bddc945a6ef5edc7551b80576249ba585b", true), "KPL-XK4R-7VJU-6EQG-7R335", "account.rs");
    assert.equal(krs.getAccountIdFromPublicKey("112e0c5748b5ea610a44a09b1ad0d2bddc945a6ef5edc7551b80576249ba585b", false), "5873880488492319831", "account.rs");
});

QUnit.test("getPrivateKey", function (assert) {
    assert.equal(krs.getPrivateKey("12345678"), "e8797c8118f02dfb649607dd5d3f8c7623048c9c063d532cc95c5ed7a898a64f", "private.key");
});

QUnit.test("encryptDecryptNote", function (assert) {
    var senderPrivateKey = "rshw9abtpsa2";
    var senderPublicKeyHex = krs.getPublicKey(converters.stringToHexString(senderPrivateKey));
    var receiverPrivateKey = "eOdBVLMgySFvyiTy8xMuRXDTr45oTzB7L5J";
    var receiverPublicKeyHex = krs.getPublicKey(converters.stringToHexString(receiverPrivateKey));
    var encryptedNote = krs.encryptNote("MyMessage", { publicKey: receiverPublicKeyHex }, senderPrivateKey);
    assert.equal(encryptedNote.message.length, 96, "message.length");
    assert.equal(encryptedNote.nonce.length, 64, "nonce.length");
    var decryptedNote = krs.decryptNote(encryptedNote.message, {
...
QUnit.test("encryptDecryptData", function (assert) {
    ...
    var senderPublicKeyHex = krs.getPublicKey(converters.stringToHexString(senderPassphrase));
    var senderPrivateKeyHex = krs.getPrivateKey(senderPassphrase);
    var receiverPassphrase = "eOdBVLMgySFvyiTy8xMuRXDTr45oTzB7L5J";
    var receiverPublicKeyHex = krs.getPublicKey(converters.stringToHexString(receiverPassphrase));
    var receiverPrivateKeyHex = krs.getPrivateKey(receiverPassphrase);
    var encryptedData = krs.encryptDataRoof(converters.stringToByteArray("MyMessage"), {
    ...
    var decryptedData = krs.decryptDataRoof(encryptedData.data, {
...
QUnit.test("getSharedKey", function (assert) {
    var privateKey = krs.getPrivateKey("rshw9abtpsa2");
    ...
    var sharedKeyBytes = krs.getSharedKey(converters.hexStringToByteArray(privateKey), converters.hexStringToByteArray(publicKey), nonceBytes);
    ...
    var decryptedMessage = krs.decryptDataRoof(converters.hexStringToByteArray(encryptedMessage), options);
