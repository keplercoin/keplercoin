...
var krs = (function (krs, $, undefined) {
        ...
        try {
            if (typeof transaction != "object") {
                krs.sendRequest("getTransaction", {
                    ...
                    krs.processTransactionModalData(response, isModalVisible, sharedKey);
                });
            } else {
                krs.processTransactionModalData(transaction, isModalVisible, sharedKey);
            }
        } catch (e) {
    ...
    krs.getPhasingDetails = function(phasingDetails, phasingParams) {
        var votingModel = krs.getVotingModelName(parseInt(phasingParams.phasingVotingModel));
        phasingDetails.votingModel = $.t(votingModel);
        switch (votingModel) {
            case 'ASSET':
                krs.sendRequest("getAsset", { "asset": phasingParams.phasingHolding }, function(response) {
                    phasingDetails.quorum = krs.convertToQNTf(phasingParams.phasingQuorum, response.decimals);
                    phasingDetails.minBalance = krs.convertToQNTf(phasingParams.phasingMinBalance, response.decimals);
                }, false);
                break;
            case 'CURRENCY':
                krs.sendRequest("getCurrency", { "currency": phasingParams.phasingHolding }, function(response) {
                    phasingDetails.quorum = krs.convertToQNTf(phasingParams.phasingQuorum, response.decimals);
                    phasingDetails.minBalance = krs.convertToQNTf(phasingParams.phasingMinBalance, response.decimals);
                }, false);
                break;              
        ...
        var phasingTransactionLink = krs.getTransactionLink(phasingParams.phasingHolding);
        if (krs.constants.VOTING_MODELS[votingModel] == krs.constants.VOTING_MODELS.ASSET) {
            phasingDetails.asset_formatted_html = phasingTransactionLink;
        } else if (krs.constants.VOTING_MODELS[votingModel] == krs.constants.VOTING_MODELS.CURRENCY) {
            phasingDetails.currency_formatted_html = phasingTransactionLink;
        }
        var minBalanceModel = krs.getMinBalanceModelName(parseInt(phasingParams.phasingMinBalanceModel));
        ...
        if (phasingParams.phasingWhitelist && phasingParams.phasingWhitelist.length > 0) {
            ...
            for (var i = 0; i < phasingParams.phasingWhitelist.length; i++) {
                var account = krs.convertNumericToRSAccountFormat(phasingParams.phasingWhitelist[i]);
                rows += "<tr><td><a href='#' data-user='" + String(account).escapeHTML() + "' class='show_account_modal_action'>" + krs.getAccountTitle(account) + "</a></td></tr>";
            }
            rows += "</tbody></table>";
        } else {
            rows = "-";
        ...
        if (phasingParams.phasingHashedSecret) {
            ...
            phasingDetails.hashAlgorithm = krs.getHashAlgorithm(phasingParams.phasingHashedSecretAlgorithm);
        }
    };

    krs.processTransactionModalData = function (transaction, isModalVisible, sharedKey) {
        krs.setBackLink();
        krs.modalStack.push({ class: "show_transaction_modal_action", key: "transaction", value: transaction.transaction });
        try {
            var async = false;
            ...
            if (transactionDetails.timestamp) {
                transactionDetails.transactionTime = krs.formatTimestamp(transactionDetails.timestamp);
            }
            if (transactionDetails.blockTimestamp) {
                transactionDetails.blockGenerationTime = krs.formatTimestamp(transactionDetails.blockTimestamp);
            }
            if (transactionDetails.height == krs.constants.MAX_INT_JAVA) {
                transactionDetails.height = "unknown";
            } else {
                transactionDetails.height_formatted_html = krs.getBlockLink(transactionDetails.height);
            ...
            $("#transaction_info_details_table").find("tbody").empty().append(krs.createInfoTable(transactionDetails, true));
            var infoTable = $("#transaction_info_table");
            infoTable.find("tbody").empty();

            var incorrect = false;
            if (transaction.senderRS == krs.accountRS) {
            ...
            var accountButton;
            if (transaction.senderRS in krs.contacts) {
                accountButton = krs.contacts[transaction.senderRS].name.escapeHTML();
            ...
            if (!transaction.attachment || !transaction.block ||
                ...
                transaction.attachment.phasingFinishHeight <= krs.lastBlockHeight) {
            ...
            if (transaction.type == krs.subtype.TaggedDataUpload.type && transaction.subtype == krs.subtype.TaggedDataUpload.subtype) {
            ...
            if (transaction.attachment && transaction.attachment.phasingFinishHeight) {
                ...
                phasingDetails.finishIn = ((finishHeight - krs.lastBlockHeight) > 0) ? (finishHeight - krs.lastBlockHeight) + " " + $.t("blocks") : $.t("finished");
                krs.getPhasingDetails(phasingDetails, transaction.attachment);
                $("#phasing_info_details_table").find("tbody").empty().append(krs.createInfoTable(phasingDetails, true));
                $("#phasing_info_details_link").show();
            } else {
                $("#phasing_info_details_link").hide();
            }
            // TODO Someday I'd like to replace it with if (krs.isOfType(transaction, "OrdinaryPayment"))
            var data;
            var message;
            var fieldsToDecrypt = {};
            var i;
            if (transaction.type == 0) {
                ...
                switch (transaction.subtype) {
                    case 0:
                        ...
                        if (transaction.attachment) {
                                ...
                                if (!transaction.attachment["version.Message"] && !transaction.attachment["version.PrunablePlainMessage"]) {
                                    try {
                                        ...
                                        if (transaction.attachment.message.indexOf("feff") === 0) {
                                            message = krs.convertFromHex16(transaction.attachment.message);
                                        } else {
                                            message = krs.convertFromHex8(transaction.attachment.message);
                                        }
                                    }
                                } else {
                            ...
                            if (transaction.attachment.encryptedMessage || (transaction.attachment.encryptToSelfMessage && krs.account == transaction.sender)) {
                                $output.append("" +
                                ...
                                if (transaction.attachment.encryptToSelfMessage && krs.account == transaction.sender) {
                                ...
                                krs.tryToDecrypt(transaction, fieldsToDecrypt, krs.getAccountForDecryption(transaction), options);
                            }
                        } else {
                        ...
                        var downloadLink = "";
                        if (transaction.attachment.messageHash && !krs.isTextMessage(transaction) && transaction.block) {
                            downloadLink = "<tr><td>" + krs.getMessageDownloadLink(transaction.transaction, sharedKey) + "</td></tr>";
                        }
                        $output.append("<table>" +
                            "<tr><td><strong>" + $.t("from") + "</strong>:&nbsp;</td><td>" + krs.getAccountLink(transaction, "sender") + "</td></tr>" +
                            "<tr><td><strong>" + $.t("to") + "</strong>:&nbsp;</td><td>" + krs.getAccountLink(transaction, "recipient") + "</td></tr>" +
                    ...
                    case 2:
                        ...
                        if (transaction.sender != krs.account) {
                            data["sender"] = krs.getAccountTitle(transaction, "sender");
                        }
                    ...
                    case 3:
                        ...
                        data = {
                            "type": $.t("vote_casting"),
                            "poll_formatted_html": krs.getTransactionLink(transaction.attachment.poll),
                            "vote": vote
                        };
                        data["sender"] = transaction.senderRS ? transaction.senderRS : transaction.sender;
                        infoTable.find("tbody").append(krs.createInfoTable(data));
                            ...
                            krs.sendRequest("getAlias", {
                                ...
                                if (!response.errorCode) {
                                    if (transaction.recipient != response.buyer || transaction.attachment.priceNQT != response.priceNQT) {
                                        ...
                                        messageStyle = "danger";
                                    } else if (transaction.recipient == krs.account) {
                                        message = $.t("alias_sale_direct_offer", {
                                            "kpl": krs.formatAmount(transaction.attachment.priceNQT)
                                    ...
                                    } else if (transaction.senderRS == krs.accountRS) {
                                        if (transaction.attachment.priceNQT != "0") {
                    ...
                    case 9:
                        ...
                        for (i = 0; i < transaction.attachment.transactionFullHashes.length; i++) {
                            ...
                            data["transaction" + (i + 1) + "_formatted_html"] =
                                krs.getTransactionLink(transactionId);
                        }
                    ...
                    case 11:
                        data = {
                            ...
                            "property_formatted_html": krs.getTransactionLink(transaction.attachment.property)
                        };
                        infoTable.find("tbody").append(krs.createInfoTable(data));
                        infoTable.show();
                ...
                switch (transaction.subtype) {
                    ...
                    case 1:
                        ...
                        krs.sendRequest("getAsset", {
                            ...
                            data = {
                                ...
                                "asset_formatted_html": krs.getTransactionLink(transaction.attachment.asset),
                            ...
                            if (data.recipient == krs.constants.GENESIS_RS) {
                    ...
                    case 3:
                        ...
                        krs.sendRequest("getAsset", {
                            ...
                            krs.formatAssetOrder(asset, transaction, isModalVisible)
                        });
                        break;
                    case 4:
                        async = true;
                            ...
                            if (transaction.attachment.asset) {
                                krs.sendRequest("getAsset", {
                                    ...
                                    data = {
                                        ...
                                        "order_formatted_html": krs.getTransactionLink(transaction.transaction),
                                        "asset_formatted_html": krs.getTransactionLink(transaction.attachment.asset),
                                        "asset_name": asset.name,
                                        "quantity": [transaction.attachment.quantityQNT, asset.decimals],
                                        "price_formatted_html": krs.formatOrderPricePerWholeQNT(transaction.attachment.priceNQT, asset.decimals) + " KPL",
                                        "total_formatted_html": krs.formatAmount(krs.calculateOrderTotalNQT(transaction.attachment.quantityQNT, transaction.attachment.priceNQT)) + " KPL"
                                    };
                                    data["sender"] = transaction.senderRS ? transaction.senderRS : transaction.sender;
                                    infoTable.find("tbody").append(krs.createInfoTable(data));
                                    infoTable.show();
                            ...
                            if (transaction.attachment.asset) {
                                krs.sendRequest("getAsset", {
                                    ...
                                    data = {
                                        ...
                                        "asset_formatted_html": krs.getTransactionLink(transaction.attachment.asset),
                                        "asset_name": asset.name,
                                        "amount_per_share": krs.formatOrderPricePerWholeQNT(transaction.attachment.amountNQTPerQNT, asset.decimals) + " KPL",
                ...
                switch (transaction.subtype) {
                    case 0:
                        data = {
                            ...
                            "quantity_formatted_html": krs.format(transaction.attachment.quantity),
                            "seller": krs.getAccountFormatted(transaction, "sender")
                        };
                    ...
                    case 1:
                        ...
                        krs.sendRequest("getDGSGood", {
                            ...
                            data = {
                                ...
                                "seller": krs.getAccountFormatted(goods, "seller")
                            };
                    ...
                    case 2:
                        ...
                        krs.sendRequest("getDGSGood", {
                            ...
                            data = {
                                ...
                                "item_name": goods.name,
                                "new_price_formatted_html": krs.formatAmount(transaction.attachment.priceNQT) + " KPL",
                                "seller": krs.getAccountFormatted(goods, "seller")
                            };
                    ...
                    case 4:
                        ...
                        krs.sendRequest("getDGSGood", {
                            ...
                            data = {
                                ...
                                "price": transaction.attachment.priceNQT,
                                "quantity_formatted_html": krs.format(transaction.attachment.quantity),
                                "buyer": krs.getAccountFormatted(transaction, "sender"),
                                "seller": krs.getAccountFormatted(goods, "seller")
                            };
                            ...
                            krs.sendRequest("getDGSPurchase", {
                                ...
                                if (purchase.errorCode) {
                                    ...
                                    if (krs.account == transaction.recipient || krs.account == transaction.sender) {
                                        if (purchase.pending) {
                                            if (krs.account == transaction.recipient) {
                    ...
                    case 5:
                        ...
                        krs.sendRequest("getDGSPurchase", {
                            ...
                            krs.sendRequest("getDGSGood", {
                                ...
                                data["quantity_formatted_html"] = krs.format(purchase.quantity);

                                if (purchase.quantity != "1") {
                                    var orderTotal = krs.formatAmount(new BigInteger(String(purchase.quantity)).multiply(new BigInteger(String(purchase.priceNQT))));
                                    data["total_formatted_html"] = orderTotal + " KPL";
                                }
                                ...
                                data["buyer"] = krs.getAccountFormatted(purchase, "buyer");
                                data["seller"] = krs.getAccountFormatted(purchase, "seller");

                                if (transaction.attachment.goodsData) {
                                    krs.tryToDecrypt(transaction, {
                                        "goodsData": {
                                            "title": $.t("data"),
                                            "nonce": "goodsNonce"
                                        }
                                    }, krs.getAccountForDecryption(purchase, "buyer", "seller"));
                                }
                                ...
                                } else if (krs.account == purchase.seller && purchase.refundNQT) {
                                    callout = $.t("purchase_refunded");
                                }
                        ...
                        krs.sendRequest("getDGSPurchase", {
                            ...
                            krs.sendRequest("getDGSGood", {
                                ...
                                data = {
                                    ...
                                    "item_name": goods.name,
                                    "buyer": krs.getAccountFormatted(purchase, "buyer"),
                                    "seller": krs.getAccountFormatted(purchase, "seller")
                                };

                                infoTable.find("tbody").append(krs.createInfoTable(data));
                                infoTable.show();

                                if (purchase.seller == krs.account || purchase.buyer == krs.account) {
                                    krs.sendRequest("getDGSPurchase", {
                                        "purchase": transaction.attachment.purchase
                                    }, function (purchase) {
                                        var callout;

                                        if (purchase.buyer == krs.account) {
                                            if (purchase.refundNQT) {
                                                callout = $.t("purchase_refunded");
                                            }
                                        } else {
                    ...
                    case 7:
                        ...
                        krs.sendRequest("getDGSPurchase", {
                            ...
                            krs.sendRequest("getDGSGood", {
                                ...
                                data["order_total_formatted_html"] = krs.formatAmount(orderTotal) + " KPL";
                ...
                switch (transaction.subtype) {
                    ...
                    case 1:
                        ...
                        krs.getPhasingDetails(data, transaction.attachment.phasingControlParams);
                ...
                switch (transaction.subtype) {
                    case 0:
                        ...
                        data = {
                            ...
                            "min_reserve_per_unit_formatted_html": krs.formatAmount(minReservePerUnitNQT) + " KPL",
                        ...
                        if (currency) {
                            data["current_units"] = krs.convertToQNTf(currency.currentSupply, currency.decimals);
                            ...
                            data["current_reserve_per_unit_formatted_html"] = krs.formatAmount(currentReservePerUnitNQT) + " KPL";
                        } else {
                    ...
                    case 1:
                        if (currency) {
                            ...
                            data = {
                                ...
                                "amount_per_unit_formatted_html": krs.formatAmount(amountPerUnitNQT) + " KPL",
                                "reserved_amount_formatted_html": krs.formatAmount(krs.calculateOrderTotalNQT(amountPerUnitNQT, krs.convertToQNTf(resSupply, currency.decimals))) + " KPL"
                            };
                        } else {
                            data = krs.getUnknownCurrencyData(transaction);
                        }
                        break;
                    case 2:
                        if (currency) {
                            ...
                            data = {
                                ...
                                "claimed_amount_formatted_html": krs.formatAmount(krs.convertToQNTf(krs.calculateOrderTotalNQT(reservePerUnitNQT, transaction.attachment.units), currency.decimals)) + " KPL"
                            };
                        } else {
                            data = krs.getUnknownCurrencyData(transaction);
                        }
                    ...
                    case 4:
                        if (currency) {
                            data = krs.formatCurrencyOffer(currency, transaction);
                        } else {
                            data = krs.getUnknownCurrencyData(transaction);
                        }
                        break;
                    case 5:
                        if (currency) {
                            data = krs.formatCurrencyExchange(currency, transaction, "buy");
                        } else {
                            data = krs.getUnknownCurrencyData(transaction);
                        }
                        break;
                    case 6:
                        if (currency) {
                            data = krs.formatCurrencyExchange(currency, transaction, "sell");
                        } else {
                            data = krs.getUnknownCurrencyData(transaction);
                        }
                        break;
                ...
                switch (transaction.subtype) {
                    case 0:
                        data = krs.getTaggedData(transaction.attachment, 0, transaction);
                        infoTable.find("tbody").append(krs.createInfoTable(data));
                        infoTable.show();

                        break;
                    case 1:
                        data = krs.getTaggedData(transaction.attachment, 1, transaction);
                        infoTable.find("tbody").append(krs.createInfoTable(data));
                        infoTable.show();
            ...
            } else if (krs.isOfType(transaction, "ShufflingCreation")) {
                data = {
                ...
                if (transaction.attachment.holding != "0") {
                    ...
                    krs.sendRequest(requestType, {"currency": transaction.attachment.holding, "asset": transaction.attachment.holding}, function (response) {
                        data.holding_formatted_html = krs.getTransactionLink(transaction.attachment.holding);
                        data.amount_formatted_html = krs.convertToQNTf(transaction.attachment.amount, response.decimals);
                    }, false);
                } else {
                    data.amount = transaction.attachment.amount;
                }
                krs.sendRequest("getShufflingParticipants", { "shuffling": transaction.transaction }, function (response) {
                    if (response.participants && response.participants.length > 0) {
                        ...
                        for (i = 0; i < response.participants.length; i++) {
                            ...
                            rows += "<tr>" +
                            "<td>" + krs.getAccountLink(participant, "account") + "<td>" +
                            "<td>" + $.t(krs.getShufflingParticipantState(participant.state).toLowerCase()) + "</td>" +
                            "</tr>";
                        }
                        rows += "</tbody></table>";
                ...
                krs.sendRequest("getShufflers", {
                    ...
                    if (response.shufflers && response.shufflers.length > 0) {
                        ...
                        data["shufflerRecipient_formatted_html"] = krs.getAccountLink(shuffler, "recipient");
                        if (shuffler.failedTransaction) {
                            data["failedTransaction_formatted_html"] = krs.getTransactionLink(shuffler.recipient);
                ...
                krs.sendRequest("getShuffling", {
                    "shuffling": transaction.transaction
                }, function (response) {
                    if (response.shuffling) {
                        data["stage_formatted_html"] = krs.getShufflingStage(response.stage);
                        data["count"] = response.registrantCount + " / " + response.participantCount;
                        data["blocksRemaining"] = response.blocksRemaining;
                        data["issuer_formatted_html"] = krs.getAccountLink(response, "issuer");
                        if (response.assignee) {
                            data["assignee_formatted_html"] = krs.getAccountLink(response, "assignee");
                        }
                ...
                infoTable.find("tbody").append(krs.createInfoTable(data));
                infoTable.show();
            } else if (krs.isOfType(transaction, "ShufflingRegistration")) {
                data = { "type": $.t("shuffling_registration") };
                krs.mergeMaps(transaction.attachment, data, { "version.ShufflingRegistration": true });
                infoTable.find("tbody").append(krs.createInfoTable(data));
                infoTable.show();
            } else if (krs.isOfType(transaction, "ShufflingProcessing")) {
                data = { "type": $.t("shuffling_processing") };
                krs.mergeMaps(transaction.attachment, data, { "version.ShufflingProcessing": true });
                infoTable.find("tbody").append(krs.createInfoTable(data));
                infoTable.show();
            } else if (krs.isOfType(transaction, "ShufflingRecipients")) {
                data = {
                    "type": $.t("shuffling_recipients"),
                    "shuffling_state_hash": transaction.attachment.shufflingStateHash
                };
                data["shuffling_formatted_html"] = krs.getTransactionLink(transaction.attachment.shuffling);
                data["recipients_formatted_html"] = listPublicKeys(transaction.attachment.recipientPublicKeys);
                infoTable.find("tbody").append(krs.createInfoTable(data));
                infoTable.show();
            } else if (krs.isOfType(transaction, "ShufflingVerification")) {
                data = { "type": $.t("shuffling_verification") };
                krs.mergeMaps(transaction.attachment, data, { "version.ShufflingVerification": true });
                infoTable.find("tbody").append(krs.createInfoTable(data));
                infoTable.show();
            } else if (krs.isOfType(transaction, "ShufflingCancellation")) {
                data = { "type": $.t("shuffling_cancellation") };
                krs.mergeMaps(transaction.attachment, data, { "version.ShufflingCancellation": true });
                infoTable.find("tbody").append(krs.createInfoTable(data));
                infoTable.show();
            }
                ...
                if (transaction.attachment) {
                    ...
                    if (transaction.attachment.message) {
                        if (!transaction.attachment["version.Message"] && !transaction.attachment["version.PrunablePlainMessage"]) {
                            ...
                            if (krs.isTextMessage(transaction)) {
                                message = String(transaction.attachment.message);
                            } else {
                                message = $.t("binary_data")
                            }
                    ...
                    if (transaction.attachment.encryptedMessage || (transaction.attachment.encryptToSelfMessage && krs.account == transaction.sender)) {
                        ...
                        if (transaction.attachment.encryptedMessage) {
                            fieldsToDecrypt.encryptedMessage = $.t("encrypted_message");
                            account = krs.getAccountForDecryption(transaction);
                        }
                        if (transaction.attachment.encryptToSelfMessage && krs.account == transaction.sender) {
                            fieldsToDecrypt.encryptToSelfMessage = $.t("note_to_self");
                            account = transaction.sender;
                        }
                        krs.tryToDecrypt(transaction, fieldsToDecrypt, account, {
    ...
    krs.formatAssetOrder = function (asset, transaction, isModalVisible) {
        var data = {
        ...
        krs.sendRequest("getOrderTrades", params, function (response) {
            ...
            if (response.trades && response.trades.length > 0) {
                ...
                for (var i = 0; i < response.trades.length; i++) {
                    ...
                    rows += "<tr>" +
                    "<td>" + krs.getTransactionLink(trade[transactionField], krs.formatTimestamp(trade.timestamp)) + "<td>" +
                    "<td>" + krs.formatQuantity(trade.quantityQNT, asset.decimals) + "</td>" +
                    "<td>" + krs.calculateOrderPricePerWholeQNT(trade.priceNQT, asset.decimals) + "</td>" +
                    "<td>" + krs.formatAmount(krs.calculateOrderTotalNQT(trade.quantityQNT, trade.priceNQT)) +
                    "</td>" +
                    "</tr>";
                }
                rows += "</tbody></table>";
            ...
            data["total_traded"] = krs.formatAmount(tradeTotal, false, true) + " KPL";
        }, null, false);
    ...
    krs.formatCurrencyExchange = function (currency, transaction, type) {
        var rateUnitsStr = " [ " + currency.code + " / KPL ]";
        var data = {
            "type": type == "sell" ? $.t("sell_currency") : $.t("buy_currency"),
            "code": currency.code,
            "units": [transaction.attachment.units, currency.decimals],
            "rate": krs.calculateOrderPricePerWholeQNT(transaction.attachment.rateNQT, currency.decimals) + rateUnitsStr
        };
        var rows = "";
        krs.sendRequest("getExchangesByExchangeRequest", {
            "transaction": transaction.transaction
        }, function (response) {
            ...
            if (response.exchanges && response.exchanges.length > 0) {
                ...
                for (var i = 0; i < response.exchanges.length; i++) {
                    ...
                    rows += "<tr>" +
                    "<td>" + krs.getTransactionLink(exchange.offer, krs.formatTimestamp(exchange.timestamp)) + "</td>" +
                    "<td>" + krs.formatQuantity(exchange.units, currency.decimals) + "</td>" +
                    "<td>" + krs.calculateOrderPricePerWholeQNT(exchange.rateNQT, currency.decimals) + "</td>" +
                    "<td>" + krs.formatAmount(krs.calculateOrderTotalNQT(exchange.units, exchange.rateNQT)) +
                    "</td>" +
                    "</tr>";
                }
                rows += "</tbody></table>";
            ...
            data["total_exchanged"] = krs.formatAmount(exchangedTotal, false, true) + " [KPL]";
        }, null, false);
        return data;
    };

    krs.formatCurrencyOffer = function (currency, transaction) {
        var rateUnitsStr = " [ " + currency.code + " / KPL ]";
        var buyOffer;
        var sellOffer;
        krs.sendRequest("getOffer", {
        ...
        if (buyOffer && sellOffer) {
            data = {
                ...
                "code": currency.code,
                "buy_supply_formatted_html": krs.formatQuantity(buyOffer.supply, currency.decimals) + " (initial: " + krs.formatQuantity(transaction.attachment.initialBuySupply, currency.decimals) + ")",
                "buy_limit_formatted_html": krs.formatQuantity(buyOffer.limit, currency.decimals) + " (initial: " + krs.formatQuantity(transaction.attachment.totalBuyLimit, currency.decimals) + ")",
                "buy_rate_formatted_html": krs.calculateOrderPricePerWholeQNT(transaction.attachment.buyRateNQT, currency.decimals) + rateUnitsStr,
                "sell_supply_formatted_html": krs.formatQuantity(sellOffer.supply, currency.decimals) + " (initial: " + krs.formatQuantity(transaction.attachment.initialSellSupply, currency.decimals) + ")",
                "sell_limit_formatted_html": krs.formatQuantity(sellOffer.limit, currency.decimals) + " (initial: " + krs.formatQuantity(transaction.attachment.totalSellLimit, currency.decimals) + ")",
                "sell_rate_formatted_html": krs.calculateOrderPricePerWholeQNT(transaction.attachment.sellRateNQT, currency.decimals) + rateUnitsStr,
        ...
        var rows = "";
        krs.sendRequest("getExchangesByOffer", {
            "offer": transaction.transaction
        }, function (response) {
            var exchangedUnits = BigInteger.ZERO;
            var exchangedTotal = BigInteger.ZERO;
    ...
    krs.getUnknownCurrencyData = function (transaction) {
        if (!transaction) {
            return {};
        }
        return {
    ...
    krs.getTaggedData = function (attachment, subtype, transaction) {
        var data = {
            "type": $.t(krs.transactionTypes[6].subTypes[subtype].i18nKeyTitle)
        };
        if (attachment.hash) {
            data["hash"] = attachment.hash;
        }
        if (attachment.taggedData) {
            data["tagged_data_formatted_html"] = krs.getTransactionLink(attachment.taggedData);
        ...
        if (attachment.data) {
            ...
            if (attachment.isText) {
                data["data_size"] = krs.getUtf8Bytes(attachment.data).length;
            } else {
                data["data_size"] = converters.hexStringToByteArray(attachment.data).length;
            }
        }
        if (transaction.block) {
            data["link_formatted_html"] = krs.getTaggedDataLink(transaction.transaction, attachment.isText);
        }
        return data;
    };
    ...
    function listPublicKeys(publicKeys) {
        ...
        for (var i = 0; i < publicKeys.length; i++) {
            ...
            var recipientAccount = {accountRS: krs.getAccountIdFromPublicKey(recipientPublicKey, true)};
            rows += "<tr>" +
                "<td>" + krs.getAccountLink(recipientAccount, "account") + "<td>" +
                "</tr>";
        }
        rows += "</tbody></table>";
        return rows;
    ...
    $(document).on("click", ".approve_transaction_btn", function (e) {
        ...
        approveTransactionModal.find('.at_transaction_timestamp').text(krs.formatTimestamp($(this).data("timestamp")));
        ...
        if ($(this).data("votingmodel") == krs.constants.VOTING_MODELS.HASH) {
