...
var krs = (function(krs, $) {
	...
	krs.connectPeer = function(peer) {
		krs.sendRequest("addPeer", {"peer": peer}, function(response) {
    ...
    krs.pages.peers = function() {
        krs.sendRequest("getPeers+", {
			...
			if (response.peers && response.peers.length) {
				...
				$("#peers_uploaded_volume").html(krs.formatVolume(uploaded)).removeClass("loading_dots");
				$("#peers_downloaded_volume").html(krs.formatVolume(downloaded)).removeClass("loading_dots");
	...
	krs.incoming.peers = function() {
		krs.loadPage("peers");
	};
	
	krs.forms.addPeerComplete = function(response) {
	...
	krs.forms.blacklistPeerComplete = function(response) {
		var message;
		var type;
	...
	function showAdminPassword(action) {
		if (!krs.needsAdminPassword) {
			...
			if (krs.getAdminPassword() != "") {
				$("#" + action + "_peer_admin_password").val(krs.getAdminPassword());
			}
		}
	}
