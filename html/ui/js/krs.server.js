...
var krs = (function (krs, $, undefined) {
    ...
    krs.setServerPassword = function (password) {
        _password = password;
    };

    krs.sendOutsideRequest = function (url, data, callback, async) {
        if ($.isFunction(data)) {
            async = callback;
            callback = data;
            data = {};
    ...
    krs.sendRequest = function (requestType, data, callback, isAsync) {
        ...
        if (!krs.isRequestTypeEnabled(requestType)) {
            callback({
                "errorCode": 1,
        ...
        if (data == undefined) {
            krs.logConsole("Undefined data for " + requestType);
            return;
        }
        if (callback == undefined) {
            krs.logConsole("Undefined callback function for " + requestType);
            return;
        }
        ...
        try {
            var KPLFields = [
                ...
                ["priceKPL", "priceNQT"],
                ["refundKPL", "refundNQT"],
                ["discountKPL", "discountNQT"],
                ["phasingQuorumKPL", "phasingQuorum"],
                ["phasingMinBalanceKPL", "phasingMinBalance"],
                ["controlQuorumKPL", "controlQuorum"],
                ["controlMinBalanceKPL", "controlMinBalance"],
                ["controlMaxFeesKPL", "controlMaxFees"],
                ["minBalanceKPL", "minBalance"],
                ["shufflingAmountKPL", "amount"],
                ["monitorAmountKPL", "amount"],
                ["monitorThresholdKPL", "threshold"]
            ];

            for (i = 0; i < KPLFields.length; i++) {
                var KPLField = KPLFields[i][0];
                var nqtField = KPLFields[i][1];
                if (KPLField in data) {
                    data[nqtField] = krs.convertToNQT(data[KPLField]);
                    delete data[KPLField];
                }
            }
        } catch (err) {
            callback({
        ...
        try {
            ...
            for (i = 0; i < currencyFields.length; i++) {
                ...
                if (decimalUnitField in data && decimalsField in data) {
                    data[field] = krs.convertToQNT(parseFloat(data[decimalUnitField]), parseInt(data[decimalsField]));
        ...
        if (requestType != "approveTransaction"
            && krs.accountInfo.accountControls && $.inArray('PHASING_ONLY', krs.accountInfo.accountControls) > -1
                && krs.accountInfo.phasingOnly
                && krs.accountInfo.phasingOnly.votingModel >= 0) {

            var phasingControl = krs.accountInfo.phasingOnly;
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": krs.convertToKPL(phasingControl.maxFees)
                    })
                });
                return;
            }
            var phasingDuration = parseInt(data.phasingFinishHeight) - krs.lastBlockHeight;
            var minDuration = parseInt(phasingControl.minDuration) > 0 ? parseInt(phasingControl.minDuration) : 0;
            var maxDuration = parseInt(phasingControl.maxDuration) > 0 ? parseInt(phasingControl.maxDuration) : krs.constants.SERVER.maxPhasingDuration;
            ...
            if (phasingDuration < minDuration || phasingDuration > maxDuration) {
                callback({
                    ...
                    "errorDescription": $.t("error_finish_height_out_of_account_control_interval", {
                        "min": krs.lastBlockHeight + minDuration,
                        "max": krs.lastBlockHeight + maxDuration
                    })
                });
                return;
            }

            var phasingParams = krs.phasingControlObjectToPhasingParams(phasingControl);
        ...
        if (requestType == "getAccountId") {
            accountId = krs.getAccountId(data.secretPhrase);

            var KPLAddress = new KplAddress();
            var accountRS = "";
            if (KPLAddress.set(accountId)) {
                accountRS = KPLAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
        ...
        if ("secretPhrase" in data) {
            accountId = krs.getAccountId(krs.rememberPassword ? _password : data.secretPhrase);
            if (accountId != krs.account && !data.calculateFee) {
                callback({
                    "errorCode": 1,
    ...
    krs.processAjaxRequest = function (requestType, data, callback, isAsync) {
        ...
        if (currentPage && krs.currentSubPage) {
            currentSubPage = krs.currentSubPage;
        }
        ...
        if (type == "POST" && krs.isRequireBlockchain(requestType) && krs.accountInfo.errorCode && krs.accountInfo.errorCode == 5) {
            callback({
                "errorCode": 2,
        ...
        var config = krs.getFileUploadConfig(requestType, data);
        ...
        $.ajax({
            ...
            if (secretPhrase && response.unsignedTransactionBytes && !data.doNotSign && !response.errorCode && !response.error) {
                var publicKey = krs.generatePublicKey(secretPhrase);
                var signature = krs.signBytes(response.unsignedTransactionBytes, converters.stringToHexString(secretPhrase));

                if (!krs.verifySignature(signature, response.unsignedTransactionBytes, publicKey, callback)) {
                    return;
                }
                addMissingData(data);
                if (file) {
                    ...
                    r.onload = function (e) {
                        ...
                        krs.verifyAndSignTransactionBytes(response.unsignedTransactionBytes, signature, requestType, data, callback, response, extra);
                    };
                ...
                if (response.errorCode || response.errorDescription || response.errorMessage || response.error) {
                    response.errorDescription = krs.translateServerError(response);
                    ...
                    if (response.broadcasted == false && !data.calculateFee) {
                        async.waterfall([
                            ...
                            krs.showRawTransactionModal(response);
                        });
                    } else {
                        if (extra) {
                            data["_extra"] = extra;
            ...
            if (krs.console) {
                krs.addToConsole(this.url, this.type, this.data, error, true);
            }
    ...
    krs.verifyAndSignTransactionBytes = function (transactionBytes, signature, requestType, data, callback, response, extra) {
        ...
        if (data.broadcast == "false") {
            ...
            krs.broadcastTransactionBytes(payload, callback, response, data);
        }
    };
    ...
    krs.verifyTransactionBytes = function (byteArray, requestType, data, attachment) {
        ...
        if (transaction.recipient !== data.recipient) {
            if ((data.recipient == krs.constants.GENESIS || data.recipient == "") && transaction.recipient == "0") {
                //ok
            } else {
                return false;
            }
        ...
        return krs.verifyTransactionTypes(byteArray, transaction, requestType, data, pos, attachment);
    };

    krs.verifyTransactionTypes = function (byteArray, transaction, requestType, data, pos, attachment) {
        var length = 0;
        var i=0;
        ...
        switch (requestType) {
            ...
            case "approveTransaction":
                ...
                if (transaction.revealedSecret !== data.revealedSecret &&
                    transaction.revealedSecret !== converters.byteArrayToHexString(krs.getUtf8Bytes(data.revealedSecretText))) {
                    return false;
                }
                break;
            ...
            case "dgsDelivery":
                ...
                if (goodsLength < 0) {
                    goodsLength &= krs.constants.MAX_INT_JAVA;
                }
                pos += 4;
            ...
            case "uploadTaggedData":
                ...
                utfBytes = krs.getUtf8Bytes(data.name);
                sha256.update(converters.byteArrayToWordArrayEx(utfBytes));
                utfBytes = krs.getUtf8Bytes(data.description);
                sha256.update(converters.byteArrayToWordArrayEx(utfBytes));
                utfBytes = krs.getUtf8Bytes(data.tags);
                sha256.update(converters.byteArrayToWordArrayEx(utfBytes));
                utfBytes = krs.getUtf8Bytes(attachment.type);
                sha256.update(converters.byteArrayToWordArrayEx(utfBytes));
                utfBytes = krs.getUtf8Bytes(data.channel);
                sha256.update(converters.byteArrayToWordArrayEx(utfBytes));
                isText = [];
                ...
                utfBytes = krs.getUtf8Bytes(data.filename);
                sha256.update(converters.byteArrayToWordArrayEx(utfBytes));
        ...
        if ((transaction.flags & position) != 0 ||
            ...
            if (messageLength < 0) {
                messageLength &= krs.constants.MAX_INT_JAVA;
            }
            pos += 4;
        ...
        if ((transaction.flags & position) != 0) {
            ...
            if (encryptedMessageLength < 0) {
                encryptedMessageLength &= krs.constants.MAX_INT_JAVA;
            }
            pos += 4;
        ...
        if ((transaction.flags & position) != 0) {
            ...
            if (encryptedToSelfMessageLength < 0) {
                encryptedToSelfMessageLength &= krs.constants.MAX_INT_JAVA;
            }
            pos += 4;
        ...
        if ((transaction.flags & position) != 0) {
            ...
            if (data.filebytes) {
                ...
                utfBytes = krs.getUtf8Bytes(data.message);
            }
            sha256.update(converters.byteArrayToWordArrayEx(utfBytes));
            hashWords = sha256.finalize();
            calculatedHash = converters.wordArrayToByteArrayEx(hashWords);
    ...
    krs.sendRequestQRCode = function(target, qrCodeData, width, height) {
        width = width || 0;
        height = height || 0;
        krs.sendRequest("encodeQRCode",
            {
    ...
    function addMissingData(data) {
        ...
        if (!("recipient" in data)) {
            data.recipient = krs.constants.GENESIS;
            data.recipientRS = krs.constants.GENESIS_RS;
        }
    }
    ...
    function validateCommonPhasingData(byteArray, pos, data, prefix) {
        ...
        for (i = 0; i < whiteListLength; i++) {
            ...
            var accountRS = krs.convertNumericToRSAccountFormat(accountId);
            pos += 8;
