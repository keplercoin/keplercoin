...
var krs = (function (krs, $, undefined) {
    krs.formatVolume = function (volume) {
    ...
    krs.formatWeight = function (weight) {
		var digits = [],
			formattedWeight = "",
			i;
		do {
    ...
    krs.formatOrderPricePerWholeQNT = function (price, decimals, zeroPad) {
		price = krs.calculateOrderPricePerWholeQNT(price, decimals, true);
		return krs.format(price, false, zeroPad);
	};

    krs.calculateOrderPricePerWholeQNT = function (price, decimals, returnAsObject) {
		if (typeof price != "object") {
			price = new BigInteger(String(price));
		}

		return krs.convertToKPL(price.multiply(new BigInteger("" + Math.pow(10, decimals))), returnAsObject);
	};

    krs.calculatePricePerWholeQNT = function (price, decimals) {
		price = String(price);
    ...
    krs.calculateOrderTotalNQT = function (quantityQNT, priceNQT) {
        return calculateOrderTotalImpl(quantityQNT, priceNQT).toString();
    };

    krs.calculateOrderTotal = function (quantityQNT, priceNQT) {
        return krs.convertToKPL(calculateOrderTotalImpl(quantityQNT, priceNQT));
    };

    krs.calculatePercentage = function (a, b, rounding_mode) {
		if (String(b) == "0") {
            return "0";
        }
		...
		Big.RM = 1;
		return krs.format(result.toString());
	};

    krs.convertToKPL = function (amount, returnAsObject) {
        var negative = "";
        var mantissa = "";
    ...
    krs.amountToPrecision = function (amount, decimals) {
    ...
    krs.convertToNQT = function (currency) {
    ...
    krs.convertToQNTf = function (quantity, decimals, returnAsObject) {
    ...
    krs.convertToQNT = function (quantity, decimals) {
		quantity = String(quantity);
    ...
    var zeros = "00000000";
    krs.format = function (params, no_escaping, zeroPad) {
        var amount;
        var mantissa;
    ...
    krs.formatQuantity = function (quantity, decimals, no_escaping, zeroPad) {
		return krs.format(krs.convertToQNTf(quantity, decimals, true), no_escaping, zeroPad);
	};

    krs.formatAmount = function (amount, round, no_escaping, zeroPad) {
        ...
        if (typeof amount == "object") {
            var params = krs.convertToKPL(amount, true);
        ...
        if (krs.settings) {
            ...
            var maxLength = parseInt(krs.settings.max_KPL_decimals) + offset;
    ...
    krs.getTransactionsAmountDecimals = function(transactions) {
        var decimals = {};
   		decimals.amount = krs.getNumberOfDecimals(transactions, "amountNQT", function (transaction) {
   			return krs.formatAmount(transaction.amountNQT);
   		});
   		decimals.fee = krs.getNumberOfDecimals(transactions, "feeNQT", function (transaction) {
   			return krs.formatAmount(transaction.feeNQT);
   		});
        return decimals;
   	};
    
    krs.getNumberOfDecimals = function(rows, key, callback) {
        var locale = krs.getLocale();
        var decimals = 0;
    ...
    //创世区块时间
    krs.fromEpochTime = function (epochTime) {
        if (!krs.constants || krs.constants.EPOCH_BEGINNING == 0) {
            throw "undefined epoch beginning";
        }
        return epochTime * 1000 + krs.constants.EPOCH_BEGINNING - 500;
    };

    krs.toEpochTime = function (currentTime) {
        if (currentTime == undefined) {
            currentTime = new Date();
        }
        if (krs.constants.EPOCH_BEGINNING == 0) {
            throw "undefined epoch beginning";
        }
        return Math.floor((currentTime - krs.constants.EPOCH_BEGINNING) / 1000);
    };

    krs.formatTimestamp = function (timestamp, date_only, isAbsoluteTime) {
        var locale = krs.getLocale();
        var date;
		if (typeof timestamp == "object") {
            ...
            date = new Date(krs.fromEpochTime(timestamp));
		}
		...
		if (!isNaN(date) && typeof(date.getFullYear) == 'function') {
            ...
            if (!date_only) {
                ...
                if (!krs.settings || krs.settings["24_hour_format"] == "0") {
                    if (originalHours == 0) {
                        hours += 12;
    ...
    krs.getBlockHeightMoment = function(height) {
        if (!height || !krs.lastBlockHeight || !krs.averageBlockGenerationTime) {
            return "-";
        }
        var heightDiff = height - krs.lastBlockHeight;
        return moment().add(heightDiff * krs.averageBlockGenerationTime, 'seconds');
    };
    
    krs.getBlockHeightTimeEstimate = function(height) {
        var heightMoment = krs.getBlockHeightMoment(height);
        if (heightMoment == "-") {
            return "-";
        }
    ...
    krs.baseTargetPercent = function(block) {
        if (block) {
    ...
    krs.convertToHex16 = function (str) {
		var hex, i;
		var result = "";
    ...
    krs.convertFromHex16 = function (hex) {
		var j;
    ...
    krs.convertFromHex8 = function (hex) {
    ...
    krs.convertToHex8 = function (str) {
		var hex = '';
    ...
    krs.getFormData = function ($form, unmodified) {
    ...
    krs.mergeMaps = function (mergedMap, toMap, skipAttributes) {
    ...
    krs.convertNumericToRSAccountFormat = function (account) {
    ...
    krs.getAccountLink = function (object, accountKey, accountRef, title, showAccountRS, clazz) {
        var accountRS;
        if (typeof object[accountKey + "RS"] != "undefined") {
            accountRS = object[accountKey + "RS"];
        } else if (typeof object[accountKey] != "undefined") {
            accountRS = krs.convertNumericToRSAccountFormat(object[accountKey]);
        } else {
            return '/';
        }
        var accountTitle;
        if (accountRef && accountRS == accountRef) {
            ...
            accountTitle = krs.getAccountTitle(object, accountKey);
        }
        if (!clazz) {
            clazz = "";
        } else {
    ...
    krs.getTransactionLink = function(id, text, isEscapedText) {
        if (!text) {
            text = id;
        }
    ...
    krs.getBlockLink = function(height, text, isEscapedText) {
        if (!text) {
            text = height;
        }
    ...
    krs.getAccountTitle = function (object, acc) {
        ...
        if (formattedAcc == krs.account || formattedAcc == krs.accountRS) {
            return $.t("you");
        } else if (formattedAcc == krs.constants.GENESIS || formattedAcc == krs.constants.GENESIS_RS) {
            return $.t("genesis");
        } else if (formattedAcc in krs.contacts) {
            return krs.contacts[formattedAcc].name.escapeHTML();
        } else {
            return String(formattedAcc).escapeHTML();
        }
    };

    krs.getAccountFormatted = function (object, acc) {
		var type = typeof object;
    ...
    krs.dataLoadFinished = function ($el, fadeIn) {
    ...
    krs.createInfoTable = function (data, fixed) {
		var rows = "";
		for (var key in data) {
            ...
            if (/_formatted_html$/i.test(key)) {
                ...
                if ($.isArray(value)) {
                    value = krs.formatQuantity(value[0], value[1]);
                } else {
                    value = krs.formatQuantity(value, 0);
                }
    ...
    krs.formatStyledAmount = function (strAmount, round) {
        var locale = krs.getLocale();
        var amount = krs.formatAmount(strAmount, round).split(locale.decimal);
		if (amount.length == 2) {
    ...
    krs.getUnconfirmedTransactionsFromCache = function (type, subtype, fields, single) {
		if (!krs.unconfirmedTransactions.length) {
			return false;
		}
		...
		for (var i = 0; i < krs.unconfirmedTransactions.length; i++) {
			var unconfirmedTransaction = krs.unconfirmedTransactions[i];
			...
			if (fields) {
				for (var key in fields) {
					...
					if (unconfirmedTransaction[key] == fields[key]) {
						if (single) {
							return krs.completeUnconfirmedTransactionDetails(unconfirmedTransaction);
						} else {
    ...
    krs.completeUnconfirmedTransactionDetails = function (unconfirmedTransaction) {
    ...
    krs.hasTransactionUpdates = function (transactions) {
		return ((transactions && transactions.length) || krs.unconfirmedTransactionsChange);
	};

    krs.showMore = function ($el) {
		if (!$el) {
			$el = $("#" + krs.currentPage + "_contents");
			if (!$el.length) {
				$el = $("#" + krs.currentPage + "_table");
			}
		}
    ...
    krs.showFullDescription = function ($el) {
		$el.addClass("open").removeClass("closed");
		$el.find(".description_toggle").text("Less...");
	};

    krs.showPartialDescription = function ($el) {
    ...
    $("body").on(".description_toggle", "click", function (e) {
		...
		if ($(this).closest(".description").hasClass("open")) {
			krs.showPartialDescription();
		} else {
			krs.showFullDescription();
		}
	});
    ...
    $("#offcanvas_toggle").on("click", function (e) {
		...
		if ($(window).width() <= 992) {
            ...
            topSide.removeClass("top-collapse-left");
		} else {
			//Else, enable content streching
            leftSide.toggleClass("collapse-left");
            rightSide.toggleClass("strech");
            topSide.toggleClass("top-collapse-left");
		}
    ...
    krs.translateServerError = function (response) {
        var match;
			...
			case 3:
				...
				if (match && match[1]) {
					return $.t("error_not_specified", {
						"name": krs.getTranslatedFieldName(match[1]).toLowerCase()
					}).capitalize();
				}
                ...
                if (match && match[1]) {
                    ...
                    for (var i=0; i<fieldNames.length; i++) {
                        translatedFieldNames.push(krs.getTranslatedFieldName(fieldNames[i].toLowerCase()));
                    }
    ...
    krs.getTranslatedFieldName = function (name) {
    ...
    krs.isControlKey = function (charCode) {
        return !(charCode >= 32 || charCode == 10 || charCode == 13);
    };

    krs.validateDecimals = function (maxFractionLength, charCode, val, e) {
        ...
        if (mantissa && mantissa[1].length > maxFractionLength) {
            var selectedText = krs.getSelectedText();
    ...
    krs.getTransactionStatusIcon = function (phasedEntity) {
        var statusIcon;
    ...
    krs.getAccountForDecryption = function(transaction, recipient, sender) {
        if (!recipient && transaction.recipient == krs.account) {
            return transaction.sender;
        }
        if (transaction[recipient] == krs.account) {
            return transaction.sender;
        }
        if (!sender && transaction.sender == krs.account) {
    ...
    krs.phasingControlObjectToPhasingParams = function(controlObj) {
        var phasingParams = {};
    ...
    krs.addEllipsis = function(str, length) {
    ...
    krs.generateToken = function(message, secretPhrase) {
        var messageBytes = krs.getUtf8Bytes(message);
        var pubKeyBytes = converters.hexStringToByteArray(krs.getPublicKey(converters.stringToHexString(secretPhrase)));
        var token = pubKeyBytes;

        var tsb = [];
        var ts = krs.toEpochTime();
        tsb[0] = ts & 0xFF;
        tsb[1] = (ts >> 8) & 0xFF;
        tsb[2] = (ts >> 16) & 0xFF;
        tsb[3] = (ts >> 24) & 0xFF;
        ...
        token = token.concat(tsb, converters.hexStringToByteArray(
            krs.signBytes(converters.byteArrayToHexString(messageBytes),
    ...
    krs.versionCompare = function (v1, v2) {
        if (v2 == undefined) {
            return -1;
