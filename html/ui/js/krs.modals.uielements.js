...
var krs = (function(krs, $, undefined) {
    ...
    function _updateBlockHeightEstimates($bhmElem) {
		...
		var output = "<i class='fa fa-clock-o'></i> " + krs.getBlockHeightTimeEstimate(blockHeight) + " ";
	...
	$("body").on("show.bs.modal", '.modal', function(e) {
		...
		$bhmElems.each(function(key, bhmElem) {
			...
			$(bhmElem).find(".bhm_ue_current_block_height").html(String(krs.lastBlockHeight));
			$(bhmElem).find(".bhm_ue_use_current_block_height").data('CurrentBlockHeight', krs.lastBlockHeight);
		});
		...
		krs.loadAlgorithmList($algoSelect, ($(this).attr('id') != 'hash_modal'));
	});
