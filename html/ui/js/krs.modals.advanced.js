...
var krs = (function(krs, $, undefined) {
	krs.showRawTransactionModal = function(transaction) {
        if (transaction.unsignedTransactionBytes && !transaction.transactionBytes) {
            ...
            krs.sendRequestQRCode("#raw_transaction_modal_unsigned_bytes_qr_code", transaction.unsignedTransactionBytes, 400, 400);
    ...
    krs.forms.broadcastTransaction = function(modal) {
        ...
        if (modal.attr('id') == "transaction_json_modal") {
            data = krs.getFormData($("#broadcast_json_form"));
        } else {
            data = krs.getFormData(modal.find("form:first"));
        }
	...
	krs.initAdvancedModalFormValues = function($modal) {
		...
		if (type != undefined && subType != undefined) {
			if (krs.transactionTypes[type]["subTypes"][subType]["serverConstants"]["isPhasingSafe"] == true) {
		...
		if (krs.accountInfo.phasingOnly) {
			minDuration = krs.accountInfo.phasingOnly.minDuration;
			maxDuration = krs.accountInfo.phasingOnly.maxDuration;
		}

		if (maxDuration == 0) {
			maxDuration = krs.constants.SERVER.maxPhasingDuration;
		}
		...
		var context = {
			...
			initBlockHeight: krs.lastBlockHeight + Math.round(minDuration + (maxDuration - minDuration) / 2),
			changeHeightBlocks: 500
		};
		var $elems = krs.initModalUIElement($modal, '.phasing_finish_height_group', 'block_height_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);

		$elems = krs.initModalUIElement($modal, '.mandatory_approval_finish_height_group', 'block_height_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);

		context = {
			labelText: "Amount KPL",
			labelI18n: "amount_KPL",
			helpI18n: "approve_transaction_amount_help",
			inputName: "phasingQuorumKPL",
			addonText: "kpl",
			addonI18n: "KPL-_unit"
		};
		$elems = krs.initModalUIElement($modal, '.approve_transaction_amount_KPL', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		$elems = krs.initModalUIElement($modal, '.approve_transaction_asset_quantity', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		$elems = krs.initModalUIElement($modal, '.approve_transaction_currency_units', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		$elems = krs.initModalUIElement($modal, '.add_approval_whitelist_group', 'multi_accounts_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		$elems = krs.initModalUIElement($modal, '.approve_min_balance_model_group', 'min_balance_model_modal_ui_element', context);
		...
		context['inputName'] = 'phasingMinBalanceKPL';
		context['addonText'] = 'KPL';
		context['addonI18n'] = 'KPL_unit';
		$elems = krs.initModalUIElement($modal, '.approve_min_balance_KPL', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
		...
		$elems = krs.initModalUIElement($modal, '.approve_min_balance_asset_quantity', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
		...
		$elems = krs.initModalUIElement($modal, '.approve_min_balance_currency_units', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
		...
		$elems = krs.initModalUIElement($modal, '.approve_holding_asset', 'add_asset_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems = krs.initModalUIElement($modal, '.approve_holding_asset_optional', 'add_asset_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
		...
		$elems = krs.initModalUIElement($modal, '.approve_holding_currency', 'add_currency_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems = krs.initModalUIElement($modal, '.approve_holding_currency_optional', 'add_currency_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		krs.initModalUIElement($modal, '.hash_algorithm_model_group', 'hash_algorithm_model_modal_ui_element', context);
	...
	krs.forms.broadcastTransactionComplete = function() {
		$("#parse_transaction_form").find(".error_message").hide();
        $("#transaction_json_modal").modal("hide");
	};

	krs.forms.parseTransactionComplete = function(response) {
		$("#parse_transaction_form").find(".error_message").hide();
        var details = $.extend({}, response);
        if (response.attachment) {
            delete details.attachment;
        }
        $("#parse_transaction_output_table").find("tbody").empty().append(krs.createInfoTable(details, true));
		$("#parse_transaction_output").show();
	};

	krs.forms.parseTransactionError = function() {
		$("#parse_transaction_output_table").find("tbody").empty();
		$("#parse_transaction_output").hide();
	};

	krs.forms.calculateFullHashComplete = function(response) {
		$("#calculate_full_hash_form").find(".error_message").hide();
		$("#calculate_full_hash_output_table").find("tbody").empty().append(krs.createInfoTable(response, true));
		$("#calculate_full_hash_output").show();
	};

	krs.forms.calculateFullHashError = function() {
    ...
    krs.forms.signTransactionComplete = function(response) {
        ...
        krs.sendRequestQRCode("#transaction_signature_qr_code", response.transactionJSON.signature, 292, 292);
        $("#signature_output").show();
    };

    krs.forms.signTransaction = function() {
        var data = krs.getFormData($("#sign_transaction_form"));
