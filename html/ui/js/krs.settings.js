...
var krs = (function(krs, $) {
	...
	krs.pages.settings = function() {
		for (var style in userStyles) {
			...
			$dropdown.empty();
			$dropdown.append("<li><a href='#' data-color=''><span class='color' style='background-color:" + krs.defaultColors[style] + ";border:1px solid black;'></span>Default</a></li>");
			...
			var color = krs.settings[style + "_color"];
			if (!color) {
				colorTitle = "Default";
			} else {
		...
		for (var key in krs.settings) {
			if (!krs.settings.hasOwnProperty(key)) {
				continue;
			}
			var setting = $("#settings_" + key);
            if (/_warning/i.test(key) && key != "asset_transfer_warning" && key != "currency_transfer_warning") {
				if (setting.length) {
					setting.val(krs.convertToKPL(krs.settings[key]));
				}
			} else if (!/_color/i.test(key)) {
				if (setting.length) {
					setting.val(krs.settings[key]);
				}
			}
		}
		...
		if (krs.database && krs.database["name"] == "krs_USER_DB") {
	...
	krs.updateStyle = function(type, color) {
		var css = "";
		var colors;
	...
	$("ul.color_scheme_editor").on("click", "li a", function(e) {
		...
		$span.html(colorTitle);
		if (color) {
			krs.updateSettings(scheme + "_color", color);
			krs.updateStyle(scheme, color);
		} else {
			krs.updateSettings(scheme + "_color");
			krs.updateStyle(scheme);
		}
	});

	krs.createLangSelect = function() {
		// Build language select box for settings page, login
		var $langSelBoxes = $('select[name="language"]');
		$langSelBoxes.empty();
		$.each(krs.languages, function(code, name) {
			$langSelBoxes.append('<option value="' + code + '">' + name + '</option>');
		});
		$langSelBoxes.val(krs.settings['language']);
	};

	krs.createRegionalFormatSelect = function() {
		// Build language select box for settings page, login
		...
		var localeKeys = krs.getLocaleList();
        for (var i=0; i < localeKeys.length; i++) {
			$regionalFormatSelBoxes.append("<option value='" + localeKeys[i] + "'>" + krs.getLocaleName(localeKeys[i]) + "</option>");
		}
		$regionalFormatSelBoxes.val(krs.settings["regional_format"]);
	};

	krs.getSettings = function(isAccountSpecific) {
		if (!krs.account) {
			krs.settings = krs.defaultSettings;
			if (krs.getStrItem("language")) {
				krs.settings["language"] = krs.getStrItem("language");
			}
			if (krs.getStrItem("themeChoice")) {
				krs.settings["themeChoice"] = krs.getStrItem("themeChoice");
			}
			krs.createLangSelect();
			krs.createRegionalFormatSelect();
			krs.applySettings();
		} else {
            async.waterfall([
                function(callback) {
					krs.storageSelect("data", [{
						...
						if (result && result.length) {
							krs.settings = $.extend({}, krs.defaultSettings, JSON.parse(result[0].contents));
						} else {
						...
						krs.logConsole("User settings for account " + krs.convertNumericToRSAccountFormat(krs.account));
						for (var setting in krs.defaultSettings) {
							if (!krs.defaultSettings.hasOwnProperty(setting)) {
								continue;
							}
							var value = krs.settings[setting];
							var status = (krs.defaultSettings[setting] !== value ? "modified" : "default");
							if (setting.search("password") >= 0) {
								value = new Array(value.length + 1).join('*');
							}
							krs.logConsole(setting + " = " + value + " [" + status + "]");
						}
						krs.applySettings();
						callback(null);
					});
                ...
                function(callback) {
                    for (var schema in krs.defaultColors) {
						if (!krs.defaultColors.hasOwnProperty(schema)) {
							continue;
						}
                        var color = krs.settings[schema + "_color"];
                        if (color) {
                            krs.updateStyle(schema, color);
                        }
                    }
                    callback(null);
                },
                function(callback) {
                    if (isAccountSpecific) {
                        krs.loadPlugins();
						if(!krs.getUrlParameter("page") || krs.getUrlParameter("page") == "dashboard") {
							krs.getAccountInfo();
	...
	krs.applySettings = function(key) {
	    if (!key || key == "themeChoice") {
			var oldlink, newlink;
			var settingsBox = $("#settings_box");
            if(krs.settings["themeChoice"] == "default"){
			...
			else if (krs.settings["themeChoice"] != ""){
				oldlink = document.getElementsByTagName("link").item(3);
				newlink = document.createElement("link");
        		newlink.setAttribute("rel", "stylesheet");
       			newlink.setAttribute("type", "text/css");
        		newlink.setAttribute("href", "css/" + krs.settings["themeChoice"] + ".css");
				document.getElementsByTagName("head").item(0).replaceChild(newlink, oldlink);
			...
			$("#change_theme").val(krs.settings["themeChoice"]);
		}
		
		if (!key || key == "language") {
			if ($.i18n.lng() != krs.settings["language"]) {
				$.i18n.setLng(krs.settings["language"], null, function() {
					$("[data-i18n]").i18n();
				});
				if (key) {
					krs.setStrItem('i18next_lng', krs.settings["language"]);
				}
			}
		}

		if (!key || key == "submit_on_enter") {
			if (krs.settings["submit_on_enter"] == "1") {
				$(".modal form:not('#decrypt_note_form_container')").on("submit.onEnter", function(e) {
					e.preventDefault();
					krs.submitForm($(this).closest(".modal"));
				});
			} else {
		...
		if (!key || key == "animate_forging") {
            ...
            if (krs.settings["animate_forging"] == "1") {
		...
		if (!key || key == "news") {
			if (krs.settings["news"] == "0") {
				$("#news_link").hide();
			} else if (krs.settings["news"] == "1") {
				$("#news_link").show();
			}
		}
		
		if (!key || key == "items_page") {
			krs.itemsPerPage = parseInt(krs.settings["items_page"], 10);
		}

		if (!krs.downloadingBlockchain) {
			if (!key || key == "console_log") {
				if (krs.settings["console_log"] == "0") {
					$("#show_console").hide();
				} else {
					$("#show_console").show();
				}
		...
		if (key == "24_hour_format") {
			...
			$.each($dashboard_dates, function() {
				$(this).html(krs.formatTimestamp($(this).data("timestamp")));
			});
		}

		if (!key || key == "remember_passphrase") {
			if (krs.settings["remember_passphrase"] == "1") {
				krs.setStrItem("remember_passphrase", 1);
			} else {
				krs.removeItem("remember_passphrase");
			}
		}
		if (!key || key == "admin_password") {
			if (krs.settings["admin_password"] != "") {
				krs.updateForgingStatus();
			}
		}
	};

	krs.updateSettings = function(key, value) {
		if (key) {
			krs.settings[key] = value;

			if (key == "themeChoice") {
				krs.setStrItem("themeChoice", value);
			}
			if (key == "language") {
				krs.setStrItem("language", value);
			}
		}

		krs.storageUpdate("data", {
			contents: JSON.stringify(krs.settings)
		}, [{
			id: "settings"
		}]);
		krs.applySettings(key);
	};

	krs.getAdminPassword = function() {
		if (window.java) {
			return window.java.getAdminPassword();
		}
		return krs.settings.admin_password;
	};
	$("#settings_box select, #welcome_panel select[name='language'], #settings_admin_password").on("change", function(e) {
		e.preventDefault();
        krs.updateSettings($(this).attr("name"), $(this).val());
	});
	...
	$("#settings_box").find("input[type=text]").on("input", function() {
		...
		var value = $(this).val();
		if (/_warning/i.test(key) && key != "asset_transfer_warning" && key != "currency_transfer_warning") {
			value = krs.convertToNQT(value);
		}
		krs.updateSettings(key, value);
	});
