...
var krs = (function(krs, $, undefined) {
    ...
    // krs.loadTransactionTypeConstants as well (below)
    krs.transactionTypes = {
        0: {
            'title': "Payment",
    ...
    krs.loadTransactionTypeConstants = function(response) {
        if (response.genesisAccountId) {
            $.each(response.transactionTypes, function(typeIndex, type) {
                if (!(typeIndex in krs.transactionTypes)) {
                    krs.transactionTypes[typeIndex] = {
                        'title': "Unknown",
                ...
                $.each(type.subtypes, function(subTypeIndex, subType) {
                    if (!(subTypeIndex in krs.transactionTypes[typeIndex]["subTypes"])) {
                        krs.transactionTypes[typeIndex]["subTypes"][subTypeIndex] = {
                            'title': "Unknown",
                            'i18nKeyTitle': 'unknown',
                            'iconHTML': '<i class="fa fa-question-circle"></i>'
                        }
                    }
                    krs.transactionTypes[typeIndex]["subTypes"][subTypeIndex]["serverConstants"] = subType;
                });
            });
            krs.subtype = response.transactionSubTypes;
        }
    };

    krs.isOfType = function(transaction, type_str) {
        if (!krs.subtype[type_str]) {
            $.growl($.t("unsupported_transaction_type"));
            return;
        }
        return transaction.type == krs.subtype[type_str].type && transaction.subtype == krs.subtype[type_str].subtype;
    };
