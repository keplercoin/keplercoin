...
var krs = (function (krs, $) {
	...
	krs.resetEncryptionState = function () {
		_password = null;
	...
	krs.generatePublicKey = function(secretPhrase) {
		if (!secretPhrase) {
		...
		return krs.getPublicKey(converters.stringToHexString(secretPhrase));
	};

	krs.getPublicKey = function(secretPhrase, isAccountNumber) {
		if (isAccountNumber) {
	...
	krs.getPrivateKey = function(secretPhrase) {
	...
	krs.getAccountId = function(secretPhrase) {
		return krs.getAccountIdFromPublicKey(krs.getPublicKey(converters.stringToHexString(secretPhrase)));
	};

	krs.getAccountIdFromPublicKey = function(publicKey, RSFormat) {
	...
	krs.getEncryptionKeys = function (options, secretPhrase){
		if (!options.sharedKey) {
			if (!options.privateKey) {
				...
				options.privateKey = converters.hexStringToByteArray(krs.getPrivateKey(secretPhrase));
			}
			...
			if (!options.publicKey) {
				...
				try {
					options.publicKey = converters.hexStringToByteArray(krs.getPublicKey(options.account, true));
				} catch (err) {
					var KPLAddress = new KplAddress();

					if (!KPLAddress.set(options.account)) {
						throw {
    ...
    krs.encryptNote = function(message, options, secretPhrase) {
		try {
			options = krs.getEncryptionKeys(options, secretPhrase);
	...
	krs.decryptData = function(data, options, secretPhrase) {
		try {
			return krs.decryptNote(message, options, secretPhrase);
		} catch (err) {
	...
	krs.decryptNote = function(message, options, secretPhrase) {
		try {
			if (!options.sharedKey) {
				if (!options.privateKey) {
					if (!secretPhrase) {
	...
	krs.getSharedKeyWithAccount = function(account) {
		try {
			...
			var privateKey = converters.hexStringToByteArray(krs.getPrivateKey(secretPhrase));

			var publicKey = converters.hexStringToByteArray(krs.getPublicKey(account, true));
	...
	krs.signBytes = function(message, secretPhrase) {
		if (!secretPhrase) {
			if (krs.rememberPassword) {
				secretPhrase = _password;
			} else {
	...
	krs.verifySignature = function(signature, message, publicKey, callback) {
	...
	krs.setEncryptionPassword = function(password) {
		_password = password;
	};

	krs.setDecryptionPassword = function(password) {
		_decryptionPassword = password;
	};

	krs.tryToDecryptMessage = function(message) {
		...
		try {
			if (!message.attachment.encryptedMessage.data) {
				...
				var decoded = krs.decryptNote(message.attachment.encryptedMessage.data, {
					"nonce": message.attachment.encryptedMessage.nonce,
					"account": (message.recipient == krs.account ? message.sender : message.recipient),
	...
	krs.tryToDecrypt = function(transaction, fields, account, options) {
			...
			$.each(fields, function(key, title) {
				...
				if (encrypted) {
					...
					try {
                        ...
                        data = krs.decryptNote(encrypted, decryptOptions);
					} catch (err) {
						if (err.errorCode && err.errorCode == 1) {
	...
	krs.removeDecryptionForm = function($modal) {
    ...
    var formatMessageArea = function (title, nrFields, data, options, transaction) {
		...
		if (data.sharedKey) {
			...
			if (!krs.isTextMessage(transaction) && transaction.block) {
				downloadLink = krs.getMessageDownloadLink(transaction.transaction, data.sharedKey) + "<br>";
			}
		}
        return "<div style='" + outputStyle + "'>" + label + "<div>" + msg + "</div>" + sharedKeyField + downloadLink + "</div>";
    };

    krs.decryptNoteFormSubmit = function() {
		...
		var accountId = krs.getAccountId(password);
		if (accountId != krs.account && !useSharedKey) {
		...
		$.each(_encryptedNote.fields, function(key, title) {
			...
			if (encrypted) {
				...
				try {
                    ...
                    data = krs.decryptNote(encrypted, options, password);
	...
	krs.decryptAllMessages = function(messages, password, sharedKey) {
		var useSharedKey = false;
		if (!password) {
			if (!sharedKey) {
				throw {
		...
		for (var i = 0; i < messages.length; i++) {
			...
			if (message.attachment.encryptedMessage && !_decryptedTransactions[message.transaction]) {
				try {
					var otherUser = (message.sender == krs.account ? message.recipient : message.sender);
					var options = {};
					if (useSharedKey) {
						options.sharedKey = converters.hexStringToByteArray(sharedKey);
					} else {
                    ...
                    var decoded = krs.decryptNote(message.attachment.encryptedMessage.data, options, password);
    ...
    krs.encryptDataRoof = function(data, options) {
	...
	krs.decryptDataRoof = function(data, options) {
    ...
    krs.sharedSecretToSharedKey = function (sharedSecret, nonce) {
        for (var i = 0; i < 32; i++) {
    ...
    krs.getSharedKey = function (privateKey, publicKey, nonce) {
		var sharedSecret = getSharedSecret(privateKey, publicKey);
        return krs.sharedSecretToSharedKey(sharedSecret, nonce);
	};

	krs.encryptFile = function(file, options, callback) {
		var r;
		try {
			r = new FileReader();
		} catch(e) {
